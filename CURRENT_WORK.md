...
**Session**: W2 SessionUpload own-liveness parity, intentionally limited to `CreateFileFromBlocks` through the pre-HEAD cut after PR #202. The new `fix/w2-sessionupload-own-liveness-v3` branch starts at `7c71449d2`; `fix/w2-sessionupload-liveness-parity` remains historical reference only. Every distinct ready placement now carries provenance, `storage_class`, and `storage_key`; the existing `up:<session>` identity is renewed or recreated before claim for both SessionUpload and BorrowedFS, then the same `ValidateBorrowedFSPublicationAuthority` LOCAL_QUORUM check runs after stage/commit immediately before HEAD CAS. No lifecycle, publication_state, terminal-witness, delete/restore, storage-accounting, quota, migration, publish-repair, R31, or X1 behavior is in this slice. See `docs/CHANGELOG.md` and `docs/PR203-SCOPE-AUDIT.md`. R31 and X1 remain OPEN. GC_ENABLED=false

**Docker validation contract (W2)**: unit, contract, and six-leg integration evidence runs are Docker-only. The existing X2/P3 multi-DC harness is a separate workflow and is unchanged by this PR. The canonical `go-integration-test` and `go-all-test` commands set `SESAMEFS_REQUIRE_SESSIONUPLOAD_OWN_LIVENESS_EVIDENCE=1` alongside the pre-existing gates; the service environments leave W2 disabled for directed runs. Directed W2 runs must unset unrelated evidence gates explicitly; the six required legs are `renewalVisibleBeforeHead`, `renewalExtendsNearExpiredTTL`, `writerFirst`, `gcFirst`, `gcFullyRetiredBeforeRenewal`, and `renewalRetryIsIdempotent`. `GC_ENABLED=false` remains required in every DC.
