...
**Session**: W2 first slice -- SessionUpload liveness parity in `CreateFileFromBlocks`, after PR #202 (W1, BorrowedFS-only) merged to `main`. `ensureBorrowedFSOwnLiveness`/`validateBorrowedFSFences` are generalized to `ensureCommitBlockOwnLiveness`/`validateCommitBlockPublicationFences`, now covering every ready block regardless of provenance: a SessionUpload block's existing `up:<session>` reference (from `/blocks/upload` time) is upserted -- renewed when present, recreated when it already lapsed -- before session claim, and the same pre-HEAD `db.ValidateBorrowedFSPublicationAuthority` exact-placement check (unchanged position, unchanged LOCAL_QUORUM consistency) now covers it too, narrowing (not closing) the "TTL alone is not a proof" gap `docs/GC-X1-PHYSICAL-LIFE-HANDOFF-PLAN.md` §14 named for this funnel -- safety through HEAD is the pre-HEAD rejection, not liveness never lapsing. A final audit also fixed a pre-existing multi-DC gap in the durable publish-repair sweep's reachability check (HEAD read + commit-ancestry walk now pinned SERIAL/EACH_QUORUM instead of plain LOCAL_QUORUM, since a stale read there authorized an irreversible commit-row/`pub:` delete). New evidence: `internal/integration/sessionupload_own_liveness_test.go` (six legs, `SESAMEFS_REQUIRE_SESSIONUPLOAD_OWN_LIVENESS_EVIDENCE=1`, wired into the canonical Docker runners) and `internal/integration/createfilefromblocks_ambiguous_head_test.go` (proves the repair/sweep converges this funnel's ambiguous-HEAD-CAS outcomes in every direction). This is a slice of W2, not its closure -- every other `CONDITIONAL`/`UNKNOWN` funnel in `docs/R3-LIVENESS-CONTINUITY.md` stays open. See `docs/CHANGELOG.md`. R31 and X1 remain OPEN. GC_ENABLED=false
