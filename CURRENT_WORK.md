...
- **r3:** also closed the generation-fence settlement gap, inverted fence order to prevent an ACTIVE window between fences, clarified that `DELETING`/`DELETED` forbids `gc_state` rollback while retaining the epoch bump, required `resolution_epoch` on the `MATERIALIZING -> VERIFIED` repair, defined `ALLOW_SUCCESSOR_AFTER_DELETE` as a new work identity after `DELETE_ALREADY_ADVANCED` (never reopen `REJECTED`), removed a verification/X2-closure contradiction about `REJECTED`, gave the post-commit abort the quarantine form matching the pointer's actual state, made a quarantined `MATERIALIZING` generation resolvable, froze the delete-escalation columns, and fixed the projection-family count plus the stale version inventory.
