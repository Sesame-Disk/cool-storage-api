...
**Session**: W2 post-HEAD publication continuity, based on main merge `f9494375e9c10e2c8d7f7766314a9d07856db89f`. This branch is intentionally limited to the shared `finalizeStoredUploadMetadata`/durable published-block-reference repair path. Lease expiry is advisory scheduling state only; background repair promotes only on positive reachability, retains every non-reachable or failed confirmation, and uses a capped age-based retry delay. The stale owner sweep is also rate-limited to a 15-minute advisory cadence. Repair settlement remains an ordinary idempotent delete, while retry backoff is process-local advisory state; neither mutates the durable repair row through Paxos. Discovery scalability remains a separate #206 follow-up. Known CAS losers are cleaned synchronously by the request that receives the definitive conflict. No lifecycle, publication-state, terminal-witness, delete/restore, storage-accounting, quota, migration, or GC-worker redesign is in this slice. Broader W2/R31 and X1 remain OPEN. `GC_ENABLED=false`.
