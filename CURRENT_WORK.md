...
**Session**: W2 post-HEAD publication continuity, based on main merge `f9494375e9c10e2c8d7f7766314a9d07856db89f`. This branch is intentionally limited to the shared `finalizeStoredUploadMetadata`/durable published-block-reference repair path. Lease expiry is advisory scheduling state only; background repair promotes only on positive reachability, retains every non-reachable or failed confirmation, and uses a capped age-based retry delay. Retry discovery now uses a durable five-minute time-wheel projection plus exact-key scheduler state, so the minute sweep does not scan all retained UNKNOWN rows or use Paxos; a startup bootstrap repairs legacy/missed schedule entries. The stale owner sweep is also rate-limited to a 15-minute advisory cadence. Known CAS losers are cleaned synchronously by the request that receives the definitive conflict. No lifecycle, publication-state, terminal-witness, delete/restore, storage-accounting, quota, or GC-worker redesign is in this slice; migration 021 is only the scheduler projection needed here. Broader W2/R31 and X1 remain OPEN. `GC_ENABLED=false`.
...
**Current W2 post-HEAD slice**: `pub:` staging and the durable repair row precede the HEAD CAS; the repair cold path reads the canonical org-scoped HEAD with SERIAL and immutable parents with EachQuorum. `onlyOfficeCommitReachable` is used only in that cold path. Positive reachability promotes `pub:` to `fs:`; every non-reachable, incomplete, or unavailable observation retains the repair row and does not actively remove its artifacts. Retry scheduling is a separate ordinary-write projection (`retry_slot` wheel plus exact-key state), with stale projection entries cleaned without authority reads and a one-time base-table bootstrap for restart/legacy recovery. The `pub:` reference still has its finite 35-day TTL; discoverable zero-ref transition remains the separate R31 follow-up. Definitive CAS losers use the request-local synchronous cleanup path. This slice does not claim W2/R31 closure for other publication funnels or broader lifecycle/GC contracts.
