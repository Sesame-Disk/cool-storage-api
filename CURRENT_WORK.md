...
**Docker validation contract (W2)**: unit, contract, mutation, and eight-leg integration evidence runs are Docker-only. The existing X2/P3 multi-DC harness is a separate workflow and is unchanged by this branch. The canonical `go-integration-test` and `go-all-test` commands set `SESAMEFS_REQUIRE_W2_POST_HEAD_EVIDENCE=1` alongside the pre-existing gates; this gate fails if the real Cassandra/MinIO run does not execute normal success, crash-after-applied-HEAD, ambiguous-applied, ambiguous-unknown-retain, lease-expiry, definite-loser cleanup, reachable-ancestor, and restart-replay legs. `GC_ENABLED=false` remains required in every DC.

**Current W2 post-HEAD slice**: `pub:` staging and the durable repair row precede the HEAD CAS; the repair cold path reads the canonical org-scoped HEAD with SERIAL and immutable parents with EachQuorum. `onlyOfficeCommitReachable` is used only in that cold path. Positive ancestor evidence promotes `pub:` to `fs:`; positive parent-ancestry evidence cleans the exact attempt; incomplete or unavailable evidence retains both. This slice does not claim W2/R31 closure for other publication funnels or broader lifecycle/GC contracts.
