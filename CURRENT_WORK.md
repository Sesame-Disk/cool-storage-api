...
7. **Current X1 key:** P1 locator authority merged 2026-08-21 (PR #181), P0/R12 landed 2026-08-23 (PR #183), and the structural P2 prerequisite landed 2026-08-24: every exact-key `BlockStore` operation enforces configured prefix + canonical org ID before backend access, while destructive locator validation is centralized. Deterministic `K == hashToKey(L)` remains; minted keys, P2, R9, R24 and X1 remain open. Keep `GC_ENABLED=false` on every replica in every DC.
