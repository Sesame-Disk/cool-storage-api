...
- Accepted the implementation contract in `docs/GC-X1-X2-GENERATION-FENCE-ADR.md` for review; **r3 is not frozen** for implementation.
...
- **r3:** closed a wedge in the quarantine-resolution abort, which failed three ways at once. It classified from an ordinary read, so a rollback could be overtaken by a live resolver or an accepted-but-unlearned pointer proposal; it fenced at most the pointer partition, while the `QUARANTINED -> null` clear names no pointer column and by correction 68 may not, so a resolver stalled before its clear survived; and it left no durable record of the abort, so a crash after the fences was indistinguishable from an ordinary claim takeover and a scanner resumed the very resolution being abandoned. The abort now **linearizes on `blocks` first**: a `QUARANTINE_ABORT` claim takeover with `retire_abort_id` is durable abort authority and pointer revocation in one LWT; only then does work move `RESOLVING -> ABORTING`, then the generation's `resolution_epoch` fence with serial settlement/idempotence, then rollback.
- **r3:** also closed the generation-fence settlement gap, inverted fence order to prevent an ACTIVE window between fences, clarified that `DELETING`/`DELETED` forbids `gc_state` rollback while retaining the epoch bump, required `resolution_epoch` on the `MATERIALIZING -> VERIFIED` repair, defined `ALLOW_SUCCESSOR_AFTER_DELETE` under immutable `work_kind=SUCCESSOR_AFTER_DELETE` after `DELETE_ALREADY_ADVANCED` (never reopen `REJECTED`, never ambiguous `OPEN`, with `OPEN -> REJECTED` cancel and post-`RESOLVING` successor-cancel abort at `abort_scope=POINTER_ONLY`), froze post-handoff single-DC ownership/routing for quarantine work-state under `LOCAL_QUORUM` (owner-down fails closed), added durable non-authoritative abort intent before `QUARANTINE_ABORT` with single-assignment logical `A` and revisable fence attempts carrying full source identity (corr 189/190), made recertification attempts revisable under stable `resolution_id` after a proven source-claim supersession and named the real `retire_abort_id` mutation set (corr 191), removed the unspecified `ABORTING -> RESOLVED` exit, removed a verification/X2-closure contradiction about `REJECTED`, gave the post-commit abort the quarantine form matching the pointer's actual state, made a quarantined `MATERIALIZING` generation resolvable, froze the delete-escalation columns, and fixed the projection-family count plus the stale version inventory.
- Kept X1/X2 open. The r3 protocol delta is documentation-only, but this is **not** a docs-only branch: it also pins the Cassandra image to 5.0.9 across the Compose files and sets `GC_ENABLED=false` explicitly in the production Compose. Destructive GC stays disabled.
...
- r3 is still not frozen: correction 191 closed the known freeze blockers from the correction-190 reviews; a dedicated freeze pass over the takeover×CAS matrix remains, plus Phase 0.
