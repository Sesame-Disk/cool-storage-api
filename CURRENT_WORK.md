...
**Docker validation contract (W2)**: unit, contract, and six-leg integration evidence runs are Docker-only. The existing X2/P3 multi-DC harness is a separate workflow and is unchanged by this PR. The canonical `go-integration-test` and `go-all-test` services set `SESAMEFS_REQUIRE_SESSIONUPLOAD_OWN_LIVENESS_EVIDENCE=1` alongside the pre-existing gates. Directed W2 runs must unset unrelated evidence gates explicitly; the six required legs are `renewalVisibleBeforeHead`, `renewalExtendsNearExpiredTTL`, `writerFirst`, `gcFirst`, `gcFullyRetiredBeforeRenewal`, and `renewalRetryIsIdempotent`. `GC_ENABLED=false` remains required in every DC.
