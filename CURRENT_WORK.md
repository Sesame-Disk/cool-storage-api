...
**Session**: W2a terminal publication authority. `libraries.publication_state` is now the shared SERIAL domain for HEAD publication and hard-delete: writers require `ACTIVE`, hard-delete commits `ACTIVE -> TERMINAL` before deleting canonical rows, and a no-TTL revocation witness survives row deletion. Publish-repair now classifies `reachable -> promote`, `ACTIVE + unreachable -> retain`, `TERMINAL -> cleanup`, and unknown/absent authority -> retain. Initial HEAD writers and deleted-library restore are also guarded by a single ACTIVE-conditioned CAS each -- **no legacy-NULL compatibility**: this is a clean deploy with no pre-existing dataset (see "pre-merge closure round 3" below), so a non-ACTIVE `publication_state` is always rejected outright, never silently promoted. The retention evidence invokes a complete synchronous sweep through an integration-only seam, rather than inferring processing from a ticker wait. Mixed-version rollout remains an operational gate: stop or isolate old writers before enabling terminal hard-delete. SessionUpload/W1 parity remains proven through HEAD but continuous `up -> pub -> HEAD -> fs` continuity remains R31. X1 remains OPEN and `GC_ENABLED=false`.

**Policy**: this codebase supports no legacy/pre-existing data of any kind, for any table, at any layer -- database rows, migrations, protocol compatibility with a prior schema version -- unless a doc explicitly says otherwise for a specific case. This is a clean deploy from an empty keyspace, not an upgrade of a live fleet. Prefer a hard rejection over a fallback that tolerates or silently promotes an unexpected/legacy-shaped state. This does not apply to client/protocol-level compatibility (e.g. legacy SHA-1 block ids from older Seafile clients, legacy S3 backend config) -- those are separate, still-valid concerns. Revisit only when X1 closes and a doc says otherwise.

**W2a follow-up (2026-09-04)**: a four-report consolidated audit found the initial-HEAD-publish cleanup (`InitializeLibraryFS`, `createInitialCommit`) deleted the content-addressed empty-root `fs_objects` row shared by every initializer of a library, so a losing concurrent initializer could delete the root a winning, already-published commit depends on — fixed by narrowing cleanup to the loser's commit key only. The same audit found the legacy-NULL HEAD-CAS retry could resurrect a hard-deleted library as `ACTIVE` (Cassandra returns the same NULL for a genuine pre-021 row and for an absent, hard-deleted partition) — fixed by gating that retry on the durable revocation witness (`db.IsLibraryPublicationRevoked`), and `createInitialCommit` gained the same legacy-NULL retry `InitializeLibraryFS` already had. The witness table's partition key was also widened to `(org_id, library_id)` before this migration ships anywhere, so a hard-deleting org does not grow one unbounded Cassandra partition. See `docs/CHANGELOG.md` "W2a follow-up" and `docs/KNOWN_ISSUES.md` for detail.

**W2a follow-up round 2 (2026-09-04)**: a second review found round 1's witness check was not atomic with the retry CAS it gated (two separate round trips against two tables), so a hard-delete landing strictly between them could still resurrect a library — closed by adding `owner_id != null` to the retry CAS's own `IF` clause in both call sites, making the existence check part of the same LWT as the retry. Also confirmed a real hazard round 1 had judged harmless: the initial commit id derived its uniqueness from `time.Now().UnixNano()`, and after round 1 narrowed loser cleanup to an exact-key delete, two initializers colliding on the same nanosecond would let the loser delete the single row the winner had just published — closed by deriving the id from a random nonce (`uuid.NewString()`) instead of the wall clock. See `docs/CHANGELOG.md` "W2a follow-up round 2" and `docs/KNOWN_ISSUES.md` for detail.

**W2a pre-merge closure (2026-09-04)**: a third review confirmed P0=0/P1=0 (including an independent re-verification of soft-delete/restore/hard-delete semantics against the actual code, not just the round-2 diff) but flagged 3 P2 closure items, all landed: (1) `TestNoLibrariesHeadCommitIDWriterBypassesPublicationAuthority` in both `internal/api` and `internal/api/v2` — an AST source guard that fails the build if any future `libraries.head_commit_id` writer omits the `publication_state` gate or `SerialConsistency(gocql.Serial)`, with the writer count pinned so a new call site is a visible diff; (2) audited every `owner_id` writer and confirmed the existence-sentinel invariant the round-2 fix depends on holds for all current code paths, documented in `docs/CHANGELOG.md`; (3) corrected stale "converges both directions" language in `docs/GC-X1-PHYSICAL-LIFE-HANDOFF-PLAN.md` (predated the round-2/root-cause commits) and the identical imprecision in the PR body. See `docs/CHANGELOG.md` "W2a pre-merge closure" for detail.

**W2a pre-merge closure round 2 (2026-09-04)**: a fourth, cross-checked review found the closure guard above had a real detection gap — it checked for `publication_state` anywhere in the whole statement, which the `SET` list always satisfies, so a mutation removing it from just the `IF` clause stayed green; it also only recognized `AssignStmt`/`ExprStmt`, missing a future `return session.Query(...)....`-shaped writer. Rewrote both packages' guard to locate the `.Query(...)` call by AST identity independent of its enclosing statement, check `publication_state` strictly after the CQL `IF` keyword, require an `IF` clause to exist at all, and require `.SerialConsistency(gocql.Serial)` chained by AST identity onto that exact call — with four new permanent mutation tests per package proving each specific failure mode goes red and a compliant (including `return`-shaped) writer stays green. Also marked `TECHNICAL-DEBT.md` §19.e RESOLVED (its "unconditional write, no concurrent writers" premise no longer holds), and — optional but cheap — added the same `owner_id != null` existence sentinel to `RevokeLibraryPublication`'s legacy-NULL CAS, so it fails instead of materializing a stub `TERMINAL` row if ever invoked against a library that was never created. See `docs/CHANGELOG.md` "W2a pre-merge closure round 2" for detail.

**W2a pre-merge closure round 3 (2026-09-04)**: a direct question cut through rounds 1-2's biggest source of complexity: this deployment has no legacy dataset and never will until X1 closes and a doc says otherwise, so the entire legacy-NULL compatibility mechanism the previous three entries spent multiple passes hardening was solving a scenario that cannot occur. Removed it outright rather than hardening it further: all four HEAD-writer functions drop their second (legacy-retry) CAS attempt down to one; `RevokeLibraryPublication` and `restoreDeletedLibrary` drop their parallel legacy branches the same way; `db.LibraryPublicationCASStateIsLegacyNull` is deleted with zero remaining references. Without a retry, `publication_state = null` is never authority: it may mean an absent partition or a malformed present row, and both fail closed. Round 2's TOCTOU class of bug has nothing left to guard against because the retry it protected no longer exists. The AST guard's pinned writer count correctly dropped from 4 to 2 per package and caught the change itself. Migration 021's own `.cql` comments were initially left untouched to avoid a checksum conflict against this session's already-migrated dev Cassandra (see round 4). See `docs/CHANGELOG.md` "W2a pre-merge closure round 3" for detail.

**W2a pre-merge closure round 4 (2026-09-04)**: round 3 fixed every CAS *writer* but a review found four *read* paths still silently treated NULL/invalid `publication_state` as ACTIVE — the same policy violation, other side of the read/write line: `db.ReadLibraryStateContext` (used transitively by the permissions middleware and block-representation resolution), `getCanonicalHeadCommitSerial` (publish-repair's cold HEAD read), `confirmLibraryHeadCommitVisible` (ambiguous-CAS confirmation), and `libraryHeadCASStateIsTerminal`/`syncLibraryHeadCASStateIsTerminal` (rejected-CAS retry classification, simplified to "ACTIVE is the only retryable value"). All four now fail closed. Also corrected the remaining "legacy-NULL compatibility" claims in `DATABASE-GUIDE.md`/`KNOWN_ISSUES.md`/`R3-LIVENESS-CONTINUITY.md`, and — since this session's dev Cassandra data volume was independently lost to a Docker Desktop restart between rounds 3 and 4, removing the checksum constraint — corrected migration 021's own comments too, then re-verified the whole suite against a freshly bootstrapped, migrated-from-scratch keyspace. See `docs/CHANGELOG.md` "W2a pre-merge closure round 4" for detail.

**Previous session**: W1 BorrowedFS own-liveness productization after the D0 X1 handoff freeze (PR #202, merged). #200 remains historical evidence; W1 productizes own `up:<session>` before claim and re-validates the exact observed physical placement (`db.ValidateBorrowedFSPublicationAuthority`, LOCAL_QUORUM -- not a bare fence check, and not the SERIAL repair boundary) immediately before HEAD. Fixed in two passes: (1) the original fence-only check could not see a block GC had already fully retired (Finalize + settled orphan); (2) that fix's first cut paid a global Paxos round trip per BorrowedFS block by reusing the SERIAL repair-authority check on this dedup hot path -- replaced with a purpose-built LOCAL_QUORUM check whose safety comes from the own `up:` pin already being durable.
...
**W2a audit consolidation (2026-09-04)**: the consolidated PR #203 report
confirmed one correctness blocker: ACTIVE plus a delete marker plus
deleted_at=NULL is not provenance for a completed restore because soft-delete
can expose those cross-table pieces at different times. The existing
hard-delete lease is now the shared library lifecycle fence for API/GC
soft-delete, restore, and hard-delete. The org cascade passes its held library
token into the fenced soft-delete path to avoid re-entrant acquisition. A
real-Cassandra regression proves a competing writer cannot consume the marker
or change the canonical row. The report's stale P4a mutation was also corrected
to target the current settlement branch. The generic lease LWTs are also
explicitly pinned to EACH_QUORUM plus global SERIAL for the production 3DC
topology. X1 remains OPEN and GC_ENABLED=false.
