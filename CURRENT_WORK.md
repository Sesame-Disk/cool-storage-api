...
7. **P1 locator authority merged 2026-08-21** (PR #181, migration `016_gc_s3_orphans_storage_key.cql`). **P0/R12 serial-phase prerequisite is now implemented on this branch**: 15 load-bearing block/orphan LWTs and 2 adjacent candidate-lifecycle LWTs pin `SERIAL` explicitly, while normal commit consistency and settlement remain future work. Neither slice satisfies X1's four closure criteria. `P2` remains the next X1 tranche and must implement minted physical incarnations before destructive GC can be reconsidered. `storage_key` remains *deterministic*: P1 changed which value is authoritative, not whether the value is recomputable, so it still cannot discriminate two physical lives at the same address.
