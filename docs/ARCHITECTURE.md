...
- **Canonical physical reads**: after SHA-1 mapping resolution, file, historic, share-link, ZIP,
  ...
  Missing metadata is retried on read paths and then fails closed; it is never interpreted as an
  arbitrary legacy backend. Session, legacy V2, and sync existence checks classify an absent row as
  missing without probing fallback S3. Encrypted audio/video explicitly advertises
  `Accept-Ranges: none`: current block metadata stores ciphertext sizes, so correct plaintext seeks
  require a future positional plaintext-size vector.
...
- **File upload**: `RegisterUploadedBlock`, in this order — one logged batch writes `AddBlockReference(up:…, TTL)` plus the canonical/by-day provisional-expiry rows → delete-fence check (`gc_state='deleting'` or `gc_s3_orphans`) → `UpsertBlockMetadata` (`INSERT IF NOT EXISTS`). The order is the point: the reference is published *before* the fence is read, so a GC worker that claims the block afterwards can observe liveness and abandon its delete. A metadata-free stub materialized by a GC claim is conditionally deleted by claim ID before retry; it is never released into an unclaimed, permanently unusable stub. Metadata is written last, only once the fence reads clear; an observed fence propagates to the outer store retry. Probe infrastructure errors consume the bounded, cancellable retry budget and remain fail-closed after exhaustion. Two limits remain: the worker only sees a reference that lands before its second `BlockHasReferences` read, and — multi-DC — `block_references` are ordinary `LOCAL_QUORUM` writes, so a reference acknowledged in one DC is not guaranteed visible to a GC read in another (see DATABASE-GUIDE).
- **Provisional expiry**: scanner Phase 0 never deletes an `up:` row. It waits until Cassandra's native TTL removes that exact referrer, then checks for other references and creates a GC candidate only when none remain. Keeping TTL as the single liveness authority avoids racing an ordinary renewal write with a conditional delete; the canonical expiry row is generation-checked only when its tracker/projection is cleaned.
