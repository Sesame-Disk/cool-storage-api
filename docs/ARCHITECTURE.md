...
- **Idempotent decrements**: `gc_processed_items` table (35d TTL) with `INSERT IF NOT EXISTS` prevents double-decrements across worker retries and DLQ requeues within the supported replay window. Task IDs are deterministic from `library_id`, `fs_object_id`, and the stable GC `identity_at`. If a retry finds an existing marker, `fs_object` deletion only proceeds when referenced block ref_counts already reflect the decrement; otherwise the worker fails closed and keeps the item recoverable.
