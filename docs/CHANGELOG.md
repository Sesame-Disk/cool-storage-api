...
Starting from main merge `f9494375e9c10e2c8d7f7766314a9d07856db89f`, the durable published-block-reference repair now settles post-HEAD outcomes explicitly. The canonical org-scoped HEAD is read in the SERIAL domain and immutable commit parents in the cold path; positive reachability promotes `pub:` to `fs:`, while every non-reachable or unavailable confirmation retains the repair and does not actively remove its artifacts. Lease expiry is retained only for compatibility/diagnostics and advisory retry scheduling; it never authorizes cleanup. Unknown rows use a capped age-based retry delay, and stale pending-owner scans run at a 15-minute advisory cadence. The latest audit's scheduler-cost finding is closed in this slice: migration 021 adds a durable five-minute time-wheel due projection and exact-key scheduler state, ordinary writes replace the retry LWT, the periodic worker reads only due projection slots, and startup bootstrap repairs legacy or missed schedule entries. Scheduler residue has cleanup paths and integration fixtures remove both the repair row and its scheduler state.
