...
Repair settlement is a conditional delete in the same pinned global SERIAL domain as retry rescheduling, so concurrent retry bookkeeping cannot resurrect a settled row. This adds one repair-row LWT per settled pending publication row, not one per block; full-scan and Paxos contention remain a separate #206 follow-up.
