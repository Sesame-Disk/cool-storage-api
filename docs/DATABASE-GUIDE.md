...
| Library HEAD publication and initial-library initialization | `SERIAL` (explicit statement pin) | Keeps HEAD advancement and terminal publication authority in one global Paxos domain. `publication_state` must be `ACTIVE` or `TERMINAL`; there is no legacy-`NULL` compatibility path (clean-deploy codebase, no pre-existing dataset — see `docs/CHANGELOG.md` "W2a pre-merge closure round 3") |
