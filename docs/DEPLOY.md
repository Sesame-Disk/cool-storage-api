...
| `SEAFHTTP_SYNC_BLOCK_MAX_INFLIGHT_PER_NODE` | `seafhttp.sync_block_max_inflight_per_node` | Concurrent block uploads allowed to hold a buffered body on one process. This is what makes `sync_block_max_bytes` an aggregate bound: buffered bodies stay under this x that cap. Derive it as (memory budget for block PUT) / (measured peak per request), not from a target request rate — the shipped `28` is 2 GiB / ~73 MiB, measured end to end. Ceiling `4096`; `0` disables the cap and gives up the aggregate bound. |
