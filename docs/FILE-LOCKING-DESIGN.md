...
1. ✅ **DONE (2026-06-19).** Enforce lock ownership in `LockFile`, **atomically via LWT** (no check-then-write race): acquire is `INSERT … IF NOT EXISTS` (`db.AcquireFileLock`) — a different user gets `409`, the owner gets a refresh; unlock is `DELETE … IF locked_by = ?` (`db.ReleaseFileLock`) — a non-owner gets `403`, a missing lock is an idempotent `200`. This closes the multi-region race where two requests could both pass a read check and then upsert. Covered by `TestLockFile_ConflictReturns409`.
   - ✅ **Phantom-lock guard (2026-06-22).** `LockFile` `lock` now verifies the target exists in the HEAD tree and is a file *before* `AcquireFileLock` (`lockTargetState`): a nonexistent path → `404`, a directory → `400`, an unresolvable/erroring traversal → `503` (fail-closed). Without this, a manual lock on a path the tree never backed (e.g. `/empty-dir/ghost.txt` or `/foo.txt/ghost`) would plant a phantom row that `SubtreeLockedByOther` honours, letting one user block legitimate rename/move/delete on the real parent. `unlock` stays lenient (a stale lock on an already-deleted path can still be cleared). Covered by `TestLockFile_RejectsNonexistentPathWith404`, `TestLockFile_RejectsDirectoryWith400`, `TestLockFile_TargetLookupFailureFailsClosedWith503`.
...
2b. ✅ **DONE (2026-06-19).** Keep lock rows consistent when the **owner** restructures a locked path (the subtree pre-check guarantees any lock under the path is the operator's own):
   ...
   - **Move** (same-repo, via `processSingleItem`/`processSameRepoMove`) → clears the source locks (the destination name is autorename-dependent, so we clear rather than guess; the owner re-locks at the destination). ✅ **Skip-aware (2026-06-22):** `processSameRepoMove` now returns `(moved bool, err error)` and `processSingleItem` clears the source locks **only when `moved == true`**. Previously a `conflict_policy=skip` move whose destination already existed was a no-op that still returned `nil`, which the caller misread as "source relocated" and silently dropped the operator's own source lock for a file that never moved. Covered by `TestProcessSingleItem_SkippedMoveDoesNotClearSourceLocks` / `TestProcessSingleItem_RealMoveClearsSourceLocks`.
...
- **Lock-check vs FS-commit is not one atomic transaction (TOCTOU window)** — every write path verifies the lock with `require(Subtree|File)NotLockedByOther` *before* entering `retryLibraryHeadMutation`, but the lock lives in a separate table (`locked_files`) acquired by its own LWT, while the structural change publishes via the library-head CAS. The two are not a single transaction, so a lock taken by another user **after** the pre-check but **before** the commit can still be bypassed: the rename/move/delete proceeds, and because the post-commit maintenance (`ClearLocksUnder` / `RelocateLocksUnder`) only touches the *operator's own* locks (filtered by `userID`), the foreign lock acquired mid-race is left orphaned on a path that no longer exists. **Accepted, not a merge blocker:** the window is narrow, the consequence is a write slipping past a just-acquired lock or an orphaned lock row (no data loss, no cross-tenant escalation), and closing it fully would require the lock check to participate in the head-CAS commit across two tables — which ScyllaDB does not make transactional. A pragmatic future mitigation (a second lock re-check immediately before `UpdateLibraryHeadFromSnapshot` inside each mutation) only *narrows* the window without closing it, and was deliberately **not** added here: it would scatter a duplicated check across every commit site (easy to forget one, against the project's centralize-side-effects rule) and pay an extra per-repo partition scan inside the retry loop on the hot path. The principled fix is a GC sweep that reconciles `locked_files` against the live tree (drops locks whose path no longer exists), tracked with the TTL/expiry work in step 3.
