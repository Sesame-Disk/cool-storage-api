...
> Phase 3/4 orphan items carry `RequiresLibraryDeletedCheck=true` and the worker guard
> revalidates against the **canonical** `libraries` table (not the `libraries_by_id`
> projection) before deleting — a live/restored/recreated library is skipped, fail-closed on
> read error (`ISSUE-GC-ORPHAN-WORKER-REVALIDATION-01`).
...
| P6b ✅ **FIXED (1E)** | **No execution-time canonical revalidation of orphan work.** Phases 3/4 enqueued orphan commit/fs_object items with `RequiresLibraryDeletedCheck=false`, so `acquireLibraryDeleteGuard` was a no-op and the worker deleted them without re-checking the canonical `libraries` row, risking removal of a live library's content under projection drift or a restore/recreate TOCTOU. **Fixed 2026-07-10:** Phase 3/4 now enqueue orphan items with `RequiresLibraryDeletedCheck=true`, and the guard revalidates against the **canonical** `libraries` table (`CanonicalLibraryExists(org,lib)`, not the `libraries_by_id` projection) — live → skip, absent → proceed, read error → fail closed. Worker + real-Cassandra tests added. (Marker-present orphans of a permanently-deleted library defer to the primary cascade / Phase 13, same class as P1.) | Med (defense-in-depth) | [scanner.go:576-590](../internal/gc/scanner.go#L576), [worker.go:1592-1608](../internal/gc/worker.go#L1592), [store_cassandra.go:2285-2302](../internal/gc/store_cassandra.go#L2285) | `ISSUE-GC-ORPHAN-WORKER-REVALIDATION-01` |
...
| 1E (P6b) ✅ **DONE** | Execution-time canonical revalidation: Phase 3/4 orphan items now carry `RequiresLibraryDeletedCheck=true`, and the worker guard re-checks the canonical `libraries` row by `(org_id, library_id)` (`CanonicalLibraryExists`, fail-closed) — live → skip, absent → proceed. Markerless P7 cleanup preserved (absent → proceed); a recreated library gets a new UUID so the old orphan's canonical row is absent (delete old content only). Worker + real-Cassandra tests. Merged 2026-07-10, branch `fix/gc-orphan-worker-revalidation`. | `ISSUE-GC-ORPHAN-WORKER-REVALIDATION-01` | Med |
