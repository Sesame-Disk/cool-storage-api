...
> - **1D (tests, new)** — the **full** suite still drains to 2 stranded blocks: one `fs:`-pinned
...
| Medium (tests) | Last 2 stranded blocks in the **full** suite | 1D (new) | Full run drains 122 blocks → **2**: one `fs:`-pinned block whose library is gone from both indexes (F1/P7 shape — **eternal**, GC idle) and one `up:sync:` provisional (self-heals in 2d via Phase 0). Dev-cluster diagnosis only |
