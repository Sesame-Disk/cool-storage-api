...
|---|---|---|---|---|
| P1 ✅ **FIXED (6A/6B)** | Org-admin trash deletes hard-deleted the library **without** enqueuing a cascade, deleting the storage counter, or cleaning tags, relying on Phase 13 (~30d). **Fixed 2026-07-13:** permanent-delete stamps `deleted_libraries.purge_requested_at` (migration 012) and Phase 13 is eligible on its next scan, so the existing `ItemLibraryCascade` reclaims content/counter/tags/policy within ~the grace period. | High (fixed) | [library_delete_helpers.go:98-116](../internal/api/v2/library_delete_helpers.go#L98), [org_admin_repos.go:390-448](../internal/api/v2/org_admin_repos.go#L390) | `ISSUE-GC-ORG-TRASH-NO-CASCADE-01` |
| P1b ✅ **FIXED (6A/6B)** | The marker is re-inserted with `deleted_at = time.Now()`, resetting the retention clock. **Fixed:** `purge_requested_at` makes eligibility independent of `deleted_at`, so the reset no longer defers the cascade. The grace gate is intentionally preserved (eligibility ≠ bypassing `grace_period`). | High (fixed) | [org_admin_repos.go:432](../internal/api/v2/org_admin_repos.go#L432), [library_delete_helpers.go:36](../internal/api/v2/library_delete_helpers.go#L36) | `ISSUE-GC-ORG-TRASH-NO-CASCADE-01` |
