...
> **Production activation update (2026-08-14):** this delete-cleanup audit predates the
> upload-fence X1/X2 findings. X2 closed 2026-08-14 under its stable-topology operational
> contract; keep `GC_ENABLED=false` on every replica in every DC while X1 remains open.
> The LWT lease does not close it. Only after X1 closes may designated replicas in one DC
> participate under the lease.
...
> Separately, X1 is the current live-data safety blocker and overrides this older scope:
...
GC disabled on every replica in every DC while upload-fence blocker X1 remains open. No reconcile/backfill pass is required before launch — there is no historical residue to
