...
| 1E (P6b) ✅ **DONE** | Execution-time orphan revalidation uses explicit durable guard modes. `canonical_absent` acquires the shared lifecycle lease and proves global canonical absence; `deleted_at_identity` preserves normal cascade semantics. Restore holds the same lease through its batch. Migration 011 and scanner→worker, cross-org, retry/DLQ, and Cassandra tests cover the contract. Completed 2026-07-10, branch `fix/gc-orphan-worker-revalidation`. | `ISSUE-GC-ORPHAN-WORKER-REVALIDATION-01` | Med |
