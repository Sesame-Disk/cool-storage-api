...
> them against the canonical `libraries` table — reachable only under `libraries_by_id`↔`libraries`
> drift, tracked as `ISSUE-GC-ORPHAN-WORKER-REVALIDATION-01`.
...
> reclamation or narrow drift-only exposure; P6a was the broad exception and is fixed.
...
| P6a ✅ **FIXED (1D)** | **Fail-open live-library classification (transient errors).** `LibraryExists`/`GroupExists` mapped every read error to `(false,nil)`, so a transient Cassandra error could make Phases 3/4 enqueue a live library's commits/fs_objects and Phase 9 delete a valid group share. **Fixed 2026-07-10:** existence reads return `(false,nil)` only for `gocql.ErrNotFound` and propagate all other errors; Phases 3/4/9 fail closed and surface the error; Phase 9 now uses the share's authoritative `OrgID`; regression tests added. | **High** | [store_cassandra.go:2270-2283](../internal/gc/store_cassandra.go#L2270), [scanner.go:531-546](../internal/gc/scanner.go#L531), [scanner.go:607-622](../internal/gc/scanner.go#L607), [scanner.go:1030-1072](../internal/gc/scanner.go#L1030), [store_cassandra.go:3208-3222](../internal/gc/store_cassandra.go#L3208) | `ISSUE-GC-EXISTENCE-CHECK-FAILOPEN-01` |
| P6b 🟠 **pending** | **No execution-time canonical revalidation of orphan work.** Phases 3/4 enqueue orphan commit/fs_object items with `RequiresLibraryDeletedCheck=false`, so `acquireLibraryDeleteGuard` is a no-op and the worker deletes them without re-checking the canonical `libraries` row. Classification is based on `libraries_by_id` (a projection); under `libraries_by_id`↔`libraries` drift (partial/manual delete, historical drift), a live library's content could be removed. Narrower than P6a (drift-only), not introduced by 1D — a pre-existing missing defense. | Med (defense-in-depth) | [scanner.go:566-574](../internal/gc/scanner.go#L566), [worker.go:1586-1589](../internal/gc/worker.go#L1586) | `ISSUE-GC-ORPHAN-WORKER-REVALIDATION-01` |
...
|---|---|---|---|
| 1D ✅ **DONE (P6a)** | Fail closed on existence reads: propagate non-`ErrNotFound` from `LibraryExists`/`GroupExists`, stop ignoring errors, Phase 9 uses the share's authoritative `OrgID`, and add scanner tests proving transient reads cannot enqueue/delete live content or shares. Merged first (2026-07-10, branch `fix/gc-existence-check-failopen`). | `ISSUE-GC-EXISTENCE-CHECK-FAILOPEN-01` | **High** |
