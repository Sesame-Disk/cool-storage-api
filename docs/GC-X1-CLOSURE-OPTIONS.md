...
- **P4b — claim → orphan publication binding (R14b, R15, R20 orphan path).** OPEN. Its
  ...
  projection-unconfirmed outcomes, reuses the stored `first_seen_at`, and settles an
  uncertain LWT with a SERIAL read. The remaining P4b work must carry P4a's exact
...
> when that settlement confirms absence; if settlement cannot establish the state, it
> remains `Ambiguous`. `SameTarget` reuses the stored `first_seen_at` and repairs the
> identity-only projection before the worker can finalize. This closes R20 for the
> publication-classification subphase, not the global orphan lifecycle: claim binding
> at publication and `FinalizeBlockDelete` remain R14b/P4b work.
