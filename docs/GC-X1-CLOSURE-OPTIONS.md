...
> remains open. Making `StartBlockDeleteOrphan` the sole creator is R21, because
> `RecordS3Orphan` still has an `INSERT … IF NOT EXISTS` path. See checklist item 10
> and the two R19 integration gates below.
...
> table TTL. `StartBlockDeleteOrphan`, `RecordS3Orphan`,
...
| **R28 schema** | The code's base-schema TTL assumption must match the effective migrated schema — `TestS3OrphanTTLConstantMatchesSchema` (base) plus `TestGC_S3OrphanEffectiveTTLMatchesMigrationChain` (integration) | **GREEN for the current migration chain.** A future TTL package must update both the constant and these gates |
