...
| R14b | The same stale lifecycle reaching **`StartBlockDeleteOrphan`** | **OPEN — P4b.** `blocks` and `gc_s3_orphans` are separate Paxos partitions, so no cross-table CAS exists; `StartBlockDeleteOrphan` is still an unconditional `INSERT … IF NOT EXISTS` that never proves the caller still owns the claim. A worker that lost ownership can therefore publish `orphan(P1)` after `P1`'s recovery completed. P4a supplies the unambiguous `(P, claimID, claimed_at)` that P4b must carry into the orphan lifecycle; it does not close this. |
