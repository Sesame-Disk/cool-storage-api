...
- **P4b — claim → orphan publication binding (R14b, R15, R20 orphan path).** OPEN. Its
  P4b-1 publication subphase is implemented: `StartBlockDeleteOrphan` is write-once,
  classifies created/same-target/different-target/not-published/ambiguous/invalid,
  projection-unconfirmed and lifecycle-advanced outcomes, reuses the stored `first_seen_at`,
  settles an uncertain LWT with a SERIAL read, and confirms canonical `EACH_QUORUM`
  visibility and `pending_s3` before `SameTarget` can authorize finalize. The remaining
  P4b work must carry P4a's exact
  authority into `StartBlockDeleteOrphan` so a worker that lost ownership cannot publish
  or mutate orphan state.
...
| `StartBlockDeleteOrphan` | `store_cassandra.go` | Write-once since P4b-1: `INSERT ... IF NOT EXISTS`, an existing row is classified and never rewritten. The reset this row was written against (*"always resets the phase to `pending_s3`"*) is gone | The overwrite that destroyed `K1`'s only durable memory is closed. Publication still cannot tell a resume of `K1` from a NEW lifecycle at the same `P`. A completed-phase same-P row is `LifecycleAdvanced` and does not authorize `FinalizeBlockDelete`; recovery may still clear that row without a physical delete. The remaining claim-binding gap is R14b |
...
R14b/P4b, the orphan-publication half, remains open. P4b-1 now classifies
`StartBlockDeleteOrphan` outcomes and keeps ambiguous, invalid and projection-unconfirmed
states fail-closed, but the publication and finalize steps still do not bind their
mutations to the claim authority. That remaining cross-partition question is P4b's,
not this row's.
...
| R14b | The same stale lifecycle reaching **`StartBlockDeleteOrphan`** | **OPEN — remaining P4b authority binding.** `blocks` and `gc_s3_orphans` are separate Paxos partitions, so no cross-table CAS exists; P4b-1 now makes the orphan publication write-once and fail-closed on uncertain classification, but it still does not prove the caller owns the claim at publication time. A worker that lost ownership can therefore publish `orphan(P1)` after `P1`'s recovery completed. P4a supplies the unambiguous `(P, claimID, claimed_at)` that the remaining P4b work must carry into the orphan lifecycle. **P4b-1 also left a second, smaller debt on this row, and it is an accepted trade rather than an oversight.** `gc_s3_orphans` is keyed by `(org_id, block_id)` alone, so publication cannot tell a resume of the current lifecycle from a NEW lifecycle that resolves to the same `P`. Write-once still will not rewind an existing row to `pending_s3`. A same-P row already at `pending_mapping_cleanup` is now `LifecycleAdvanced`: `processBlock` does not `FinalizeBlockDelete`, because recovery on that phase does not consult `BlockExists` and clearing the orphan after dropping `blocks` is a live-content window. Recovery may still finalize that completed-phase row without a physical delete. For the stale-phase-reset ambiguity itself, write-once stays leak-biased (recovery will not aim a second delete at a key a new incarnation may have re-created). Full loss-freedom still requires R14b orphan-lifecycle authority binding. `SameTarget` confirmation proves visibility now; it does not renew the row's remaining TTL (R27/R28). |
| R15 | `StartBlockDeleteOrphan` returns a conflict or ambiguous error after the orphan insert may have applied | **P4b-1 implemented for publication classification.** Same-target is an idempotent resume using the stored lifecycle token; it confirms the canonical row at `EACH_QUORUM`, requires `recovery_phase = pending_s3`, and then repairs the identity-only projection before the worker can finalize. A visible same-P row past `pending_s3` is `LifecycleAdvanced` and does not authorize destructive completion. Different-target is a confirmed competing lifecycle; not-published, ambiguous, invalid and projection-unconfirmed outcomes also do not. A timeout or error is settled with a SERIAL read, never by an ordinary consistency read. An empty non-applied CAS map is settled the same way and is never treated as absence. The remaining claim-binding gap is R14b. |
...
> `SameTarget` reuses the stored `first_seen_at`, confirms the canonical row at
...
> for the publication-classification subphase, not the global orphan lifecycle: claim
> binding at publication and `FinalizeBlockDelete` remain R14b/P4b work.
