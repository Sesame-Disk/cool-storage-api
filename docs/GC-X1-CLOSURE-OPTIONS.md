...
0b. **R25 — every promote path must have staged a checked `pub:`.** Ranked with R17
   ...
   than by a decision: `TestSyncPublishPathsStageBeforePromoting`.
...
| Leg | Property | Instrument | Status |
|---|---|---|---|
| R25 handshake | Every sync entry point that promotes permanent `fs:` first staged a `pub:` for the same attempt | `TestSyncPublishPathsStageBeforePromoting` (`internal/api/sync_publish_handshake_test.go`) | **RED** on the idempotent-retry case; the other three entry points pass |
...
| R3 safety | A block under an active delete fence must not gain a permanent `fs:` reference through the retry path | not built | Needs a second instance for a cold memo; cannot go green until R3's post-check exists on any path |
...
**The mutation is what makes a red meaningful.** The handshake leg was verified both ways:
red as the code stands, green when `repairPublishedSyncCommitBlockDelta` is changed to go
through `stageSyncCommitBlockDelta`, with the memoized fast path still passing (it promotes
nothing, so it satisfies the invariant trivially — that case exists so a fix which simply
always re-stages does not silently make every idempotent retry pay the full reconciliation).
