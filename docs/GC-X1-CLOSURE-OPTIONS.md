...
- **P4b — claim → orphan publication binding (R14b, R15, R20 orphan path).** OPEN. Its
  ...
  classifies created/same-target/different-target/not-published/ambiguous/invalid and
  projection-unconfirmed outcomes, reuses the stored `first_seen_at`, settles an
  uncertain LWT with a SERIAL read, and confirms canonical `EACH_QUORUM` visibility
  before `SameTarget` can authorize finalize. The remaining P4b work must carry P4a's exact
...
| `StartBlockDeleteOrphan` | `store_cassandra.go` | Write-once since P4b-1: `INSERT ... IF NOT EXISTS`, an existing row is classified and never rewritten. The reset this row was written against (*"always resets the phase to `pending_s3`"*) is gone | The overwrite that destroyed `K1`'s only durable memory is closed. What remains is the other half: publication still cannot tell a resume of `K1` from a NEW lifecycle at the same `P`, so it resolves that ambiguity toward resume and can inherit a completed phase ⇒ leaked object. Needs the incarnation, i.e. R14b |
...
| R14b | The same stale lifecycle reaching **`StartBlockDeleteOrphan`** | **OPEN — remaining P4b authority binding.** `blocks` and `gc_s3_orphans` are separate Paxos partitions, so no cross-table CAS exists; P4b-1 now makes the orphan publication write-once and fail-closed on uncertain classification, but it still does not prove the caller owns the claim at publication time. A worker that lost ownership can therefore publish `orphan(P1)` after `P1`'s recovery completed. P4a supplies the unambiguous `(P, claimID, claimed_at)` that the remaining P4b work must carry into the orphan lifecycle. **P4b-1 also left a second, smaller debt on this row, and it is an accepted trade rather than an oversight.** `gc_s3_orphans` is keyed by `(org_id, block_id)` alone, so publication cannot tell a resume of the current lifecycle from a NEW lifecycle that resolves to the same `P`. Write-once resolves that ambiguity toward "resume": it no longer rewinds an existing row to `pending_s3`, so a block re-uploaded while its old row sat at `pending_mapping_cleanup` and then condemned again inherits the completed phase, and recovery skips the physical delete. The cost is a **leaked** object if the new lifecycle does not complete its physical S3 delete after inheriting `pending_mapping_cleanup` — by crash or cancellation between publication and `processBlock`'s inline delete, **or** by that inline delete exhausting its retries (`UpdateS3OrphanAttempt` does not rewind the phase, the queue item still completes, and recovery treats `pending_mapping_cleanup` as "S3 already succeeded"). The rewind it replaced could aim a second delete at a key a new incarnation had re-created, which is **loss**. The incarnation binding this row already requires is what removes the ambiguity; nothing smaller does. |
| R15 | `StartBlockDeleteOrphan` returns a conflict or ambiguous error after the orphan insert may have applied | **P4b-1 implemented for publication classification.** Same-target is an idempotent resume using the stored lifecycle token; it confirms the canonical row at `EACH_QUORUM` and then repairs the identity-only projection before the worker can finalize. Different-target is a confirmed competing lifecycle; not-published, ambiguous, invalid and projection-unconfirmed outcomes do not authorize destructive completion. A timeout or error is settled with a SERIAL read, never by an ordinary consistency read. An empty non-applied CAS map is settled the same way and is never treated as absence. The remaining claim-binding gap is R14b. |
...
> `EACH_QUORUM` (so a `LOCAL_QUORUM` writer in every DC can see the fence), and then
> repairs the identity-only projection before the worker can finalize. This closes R20
