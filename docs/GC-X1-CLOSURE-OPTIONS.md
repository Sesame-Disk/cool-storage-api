...
**P0/R12 serial-phase prerequisite (2026-08-23):** The 10 conditional statements
on `blocks` (count source: `r12ExpectedSerialOperations`) and the four conditional
statements on canonical `gc_s3_orphans` now pin `SerialConsistency(gocql.Serial)`
explicitly. P4b-2 audit closures add two conditional statements on
`gc_block_delete_lifecycles`. The candidate mutations are pinned in the same
hardening slice. This changes only the Paxos serial phase relative to P0: regular
commit consistency, serial settlement, exact physical identity and X1 closure
remain open.
...
and the writer publication gap R3 is GREEN (post-stage check). The R22 row below records the original
...
straggler invalidates every other statement's guarantee — see R12's inventory of ten
`blocks` statements (`r12ExpectedSerialOperations` is the count source). That must
land before any install property depends on a `SERIAL` LWT/Paxos
transaction, and it
...
- **P4a — exact-`P`, per-attempt claim authority (R14a, R16, R20 claim path).** ✅ IMPLEMENTED.
  Candidate, claim, release, stale takeover, finalize and candidate cleanup are all bound to
  the exact `(storage_class, storage_key)` and to a fresh per-attempt `claimID` with its
  `claimed_at`. Deliberately does NOT touch the orphan lifecycle.
- **P4b — claim → orphan publication binding (R14b, R15, R20 orphan path).** ✅ IMPLEMENTED
  (P4b-1 write-once + P4b-2 authority handoff). `StartBlockDeleteOrphan` is write-once,
  classifies created/same-authority/different-target/different-authority/unbound/not-published/
  ambiguous/invalid/projection-unconfirmed/lifecycle-advanced, reuses stored `first_seen_at`,
  settles an uncertain LWT with a SERIAL read, and confirms canonical `EACH_QUORUM`
  visibility, `pending_s3`, and exact `(P, D)` before `SameAuthority` can authorize finalize.
  `CommitBlockDeleteOrphanHandoff` is the irreversible `blocks` marker that binds that
  publication and `FinalizeBlockDelete` to the stored claim. `AlreadyCommitted` /
  `CommittedOwner` require canonical `EACH_QUORUM` visibility. After handoff,
  `CommittedOwner` treats `BlockHasReferencesGlobal` as a contradiction detector
  (R3 is the writer post-check, not this detector). `StartBlockDeleteOrphan` INSERTs `gc_block_delete_lifecycles`
  first (migration `020`, not folded into `001`); a SERIAL post-check after the
  orphan INSERT returns `LifecycleAdvanced` if D has already gone terminal.
  `AlreadyFinalized` is a classification that the certificate is still `published`;
  only applied `Finalized` authorizes `processBlock` S3. `RecoverS3Orphans`
  `pending_s3` SERIAL-observes D and will not DELETE bytes against a terminal
  tombstone (it may clear a stale orphan). X1 remains OPEN — the winner of
  `DELETE blocks` pausing after authorization is not closed here.
  `GC_ENABLED=false` remains mandatory.
...
| `pub:` then `fs:` | `StagePublishAttemptReferences` / `PromotePublishAttemptReferences` | Publish-attempt liveness across the HEAD CAS | **R3 GREEN:** post-stage authority check after a complete `pub:` stage; only Active continues to HEAD/promote. `Promote` stays dumb. Crash leftover `pub:` is **R18**. Durable repair of published HEAD is **R31**. |
...
| `StartBlockDeleteOrphan` | `store_cassandra.go` | Write-once since P4b-1; P4b-2 persists D and classifies `SameAuthority` / `DifferentAuthority`. The reset this row was written against (*"always resets the phase to `pending_s3`"*) is gone | The overwrite that destroyed `K1`'s only durable memory is closed. A competing D at the same `P` is `DifferentAuthority`, not a resume. A completed-phase same-P row is `LifecycleAdvanced` and does not authorize `FinalizeBlockDelete`; recovery may still clear that row without a physical delete. Two lives of one logical block still share one orphan PK (P4c-orphan) |
...
A fifth pass extended the late-loser rule from settlement to QUEUE POLICY. Five post-claim
unwinds released the fence and returned an ordinary error while discarding the release
...
R14b/P4b is GREEN as of P4b-2. P4b-1 classified `StartBlockDeleteOrphan` and kept
uncertain publication fail-closed; P4b-2 binds that publication and finalize to the
exact `(P, D)` committed on `blocks` via `gc_orphan_handoff`. Cross-table CAS still
does not exist — the durable marker plus matching D on both partitions is the binding.
The orphan PRIMARY KEY remains `(org_id, block_id)` (P4c-orphan).
...
| R3 | Writer stalled after `Probe=Reusable` (LQ, no fence yet); GC fences, sees 0, deletes `K1`; the writer then stages `pub:` | ✅ **IMPLEMENTED.** Post-stage `ValidatePublishAttemptAuthority` after a complete `pub:` write (`StagePublishAttemptReferences` and `stagePendingPublishedFiles`). Only Active continues; any other outcome rolls back **this attempt's** `pub:` rows. Rollback failure is not publication success. v2 durable repair (`publish_repair.go`) is promote-only (HEAD may already be published). R18/R27/R29 integration/R30/R31 remain OPEN. |
...
| R12 | Any conditional statement on the `blocks` partition still runs at `LOCAL_SERIAL` after the others are raised | **Fails the whole fence.** The two levels are different quorum domains, so a `LOCAL_SERIAL` round can miss an in-flight `SERIAL` proposal and one straggler invalidates every other statement's guarantee. P0 pins the inventory in `r12ExpectedSerialOperations`; P4b-2 adds `CommitBlockDeleteOrphanHandoff` (10 conditional `blocks` statements — do not re-count from stale prose — plus 4 canonical orphan, 3 candidate, and 2 lifecycle-tombstone LWTs). |
...
| R14a | A candidate enqueued for `P1=(B1,K1)` is processed after `P1` died and `P2` was installed — **candidate → claim → release/stale-takeover → finalize** | ✅ **IMPLEMENTED IN P4a (GREEN).** `gc_block_candidates` and its `_by_day` projection persist `storage_key` (migration 017) and `EnsureBlockGCCandidate` captures it from the canonical row, refusing to write a candidate it cannot name a `P` for. The claim CAS is `IF storage_class = ? AND storage_key = ? AND gc_state = null AND gc_claim_id = null AND gc_claimed_at = null`; release, stale takeover and finalize each condition on the exact `(P, claimID, claimed_at)`; and `DeleteBlockGCCandidate` is a CAS on the candidate's own `(storage_class, storage_key, candidate_at)`, so a late `P1` lifecycle can neither act on `P2` nor consume `P2`'s work item. `claimID` is a fresh UUID per ATTEMPT — `blockDeleteClaimID(candidateAt)` is deleted, and its absence is gated. Two side effects worth recording: requiring all three `gc_*` columns null keeps GC off the upload path's `repairing_stub`, and it removes stub materialization at the root (an `IF` naming `storage_class` cannot apply against a missing partition), so the worker's stub-cleanup branch is now unreachable for candidates. Two things a first reading of this row would get wrong, both corrected before merge. **The stale takeover is a CAS against the authority the claim OBSERVED**, carried back on `BlockClaimResult` — not a second read that releases whichever owner it finds, which would let a worker authorized for `P1` drop `P2` fence; the same binding was added to the owner-agnostic pre-check release, where the cross-incarnation case needs no clock skew at all, just an ordinary re-mint. **And the destructive locator is `attempt.Target`, not a post-claim `GetBlockInfo` re-read**: that read is ordinary while the claim commits at EACH_QUORUM in the serial domain, so sourcing the orphan publication and the S3 delete from it was the same re-read-and-destroy shape one step below the claim. A divergence between the two now releases the claim and fails closed. Evidence: `TestP4A_*` unit legs, real-Cassandra `internal/integration/p4a_claim_authority_test.go` under `SESAMEFS_REQUIRE_P4A_EVIDENCE=1`, source guards in `internal/gc/p4a_claim_authority_guard_test.go`, and the deliberate mutations red via `scripts/p4a-mutation-validation.sh` — see the P4a evidence update above for the current count and the passes that followed this row; the numbers once written here (seventeen, later twenty-one) are stale. |
| R14b | The same stale lifecycle reaching **`StartBlockDeleteOrphan`** | ✅ **IMPLEMENTED IN P4b-2 (GREEN), audit-hardened.** `blocks` and `gc_s3_orphans` remain separate Paxos partitions, so there is still no cross-table CAS. Binding is the durable `blocks.gc_orphan_handoff=true` commit (`CommitBlockDeleteOrphanHandoff`, exact `(P,D)`, handoff still null) plus the same D persisted on the write-once orphan INSERT. `AlreadyCommitted` / `CommittedOwner` are returned only after canonical `EACH_QUORUM` visibility; an empty non-applied handoff CAS SERIAL-settles. After that commit, release/stale takeover/new D are refused; `ClaimBlockDelete` classifies `CommittedOwner` and resumes stored D; `CommittedOwner` re-checks refs as a contradiction detector (R3 is the writer post-check, not this detector). `StartBlockDeleteOrphan` INSERTs `gc_block_delete_lifecycles` (`published`) first (migration `020`, not folded into `001`) and SERIAL-re-reads that tombstone after the orphan INSERT; terminal D is `LifecycleAdvanced` and cannot recreate an authorizing orphan. `FinalizeBlockDelete` requires `gc_orphan_handoff=true` plus exact `(P,D)`. `AlreadyFinalized` requires an exact published `(P,D)` certificate and is **not** permission to delete bytes — only applied `Finalized` authorizes `processBlock` S3. Missing, mismatched, or garbage lifecycle certificates fail closed. `RecoverS3Orphans` `pending_s3` SERIAL-observes D before S3 and will not DELETE against terminal (it may clear a stale orphan). Production `DeleteS3Orphan` runs only after `published → terminal`. Post-commit failures leave the queue untouched. P4b-1 write-once, SERIAL settlement, `pending_s3`, and canonical `EACH_QUORUM` remain. **Still not this row:** the orphan PRIMARY KEY is `(org_id, block_id)` (P4c-orphan); two lives of one logical block still cannot occupy two orphan rows; the winner of `DELETE blocks` pausing after authorization is X1 Physical ABA, still OPEN; never-delete `gc_block_delete_lifecycles` partitions grow by one row per D (operational follow-up, not a merge blocker). Evidence: `internal/gc/p4b_authority_handoff_test.go`, `internal/gc/p4b_lifecycle_tombstone_test.go`, `scripts/p4b-authority-mutation-validation.sh`, real Cassandra `internal/integration/p4b_claim_orphan_authority_test.go` under `SESAMEFS_REQUIRE_P4B_EVIDENCE=1`. X1 stays OPEN. `GC_ENABLED=false`. |
| R15 | `StartBlockDeleteOrphan` returns a conflict or ambiguous error after the orphan insert may have applied | **P4b-1 + P4b-2.** Same-authority is an idempotent resume using the stored lifecycle token and the stored D; it confirms the canonical row at `EACH_QUORUM`, requires `recovery_phase = pending_s3` and exact `(P,D)`, and then repairs the identity-only projection before the worker can finalize. Same P / different D is `DifferentAuthority`, never success. A visible same-P row past `pending_s3` is `LifecycleAdvanced` and does not authorize destructive completion. Different-target, unbound, not-published, ambiguous, invalid and projection-unconfirmed outcomes also do not. A timeout or error is settled with a SERIAL read, never by an ordinary consistency read. An empty non-applied CAS map is settled the same way and is never treated as absence. Claim binding at publication is R14b/P4b-2. |
| R16 | A fresh attempt `D2` calls `ClaimBlockDelete(P1,D2)` while `P1` is already claimed by fresh `D1` | `!applied` is not completion. Classify row absent, same `P1` with fresh `D1` (postpone and preserve the candidate), same `P1` with stale `D1` (take over with CAS), different `P` (stale candidate; never touch it), and ambiguous timeout (settle serially per R20; no candidate cleanup). A fresh claim UUID invalidates the current `!applied → DeleteBlockGCCandidate` shortcut. ✅ **IMPLEMENTED IN P4a (GREEN).** `ClaimBlockDelete` returns a classified `BlockClaimOutcome` — `acquired`, `target_changed`, `fresh_owner`, `stale_owner`, `missing`, `invalid`, `ambiguous` — and `processBlock` acts on each differently. The `!applied → DeleteBlockGCCandidate` shortcut is gone: `fresh_owner` postpones without spending a retry and preserves the candidate, `target_changed` settles only the stale candidate by its own exact identity, and `invalid`/`ambiguous` mutate nothing and consume nothing. Gated by `TestP4A_FreshOwnerDoesNotSettleTheCandidate` and the `m_fresh_owner_completes_candidate` mutation. **Both exits are closed, and there are three, not two.** `BlockClaimFreshOwner` refuses to create a fence with no work item at the CLAIM; the re-referenced branch refuses to consume the current owner's candidate at SETTLEMENT; and, since 2026-08-27, no post-claim unwind may spend the item's retry budget after a not-owner release — a preserved candidate whose only work item is in a DLQ `ItemBlock` never leaves is the same stranded state reached by a third door. |
...
> **P4b-1 R20 update (2026-08-27):** `StartBlockDeleteOrphan` now settles an
> uncertain LWT with a query-consistency `SERIAL` read. It returns `NotPublished` only
> when that settlement confirms absence — an empty non-applied CAS map is not absence
> and is settled the same way. If settlement cannot establish the state, it remains
> `Ambiguous`. `Created` already proved canonical `EACH_QUORUM` on the LWT.
> `SameAuthority` reuses the stored `first_seen_at`, confirms the canonical row at
> `EACH_QUORUM` (so a `LOCAL_QUORUM` writer in every DC can see the fence), requires
> `pending_s3`, and then repairs the identity-only projection before the worker can
> finalize. That confirmation does not renew the row's remaining TTL. This closes R20
> for the publication-classification subphase. P4b-2 then binds that publication and
> `FinalizeBlockDelete` to the committed `(P, D)` (R14b GREEN).
> Any older matrix wording that describes `StartBlockDeleteOrphan` as resetting a
> lifecycle is the pre-P4b baseline, not the current implementation.
...
> mismatches against a differently-tokened row. `StartBlockDeleteOrphan` is now
> write-once: a same-target retry preserves the existing token and recovery state,
> while a different target is not overwritten. Lifecycle binding of the later phase
> and clear mutations remains open. R21 is now closed: `StartBlockDeleteOrphan` is the sole production
...
gated; R3's post-stage check is now GREEN (item 8), and the structural fix is not by itself X1 closure.
...
ten.** R12's own rule is that one statement left behind invalidates the guarantee for
all the others, so the count is not a detail. **`r12ExpectedSerialOperations` is the
source of truth** — do not restore a stale prose “eleven”.

| # | Statement | Location |
|---|---|---|
| 1 | `installBlockMetadataLWTFn` — first-writer `INSERT … IF NOT EXISTS` | `internal/db/block_references.go` |
| 2 | `claimReleasedBlockStubForRepairFn` | `internal/db/block_references.go` |
| 3 | `deleteRepairClaimedBlockStubFn` | `internal/db/block_references.go` |
| 4 | `deleteClaimedBlockStubFn` | `internal/db/block_references.go` |
...
| 7 | `ClaimBlockDelete` | `internal/gc/store_cassandra.go` |
| 8 | `ReleaseBlockClaim` | `internal/gc/store_cassandra.go` |
| 9 | `CommitBlockDeleteOrphanHandoff` | `internal/gc/store_cassandra.go` |
| 10 | `FinalizeBlockDelete` | `internal/gc/store_cassandra.go` |

`ReleaseStaleBlockClaim` is a SERIAL observing read, not a conditional mutation.
`ReleaseBlockDeleteClaim` is not in the allowlist. P4b-2 audit closures add two
conditional statements on `gc_block_delete_lifecycles` (INSERT tombstone, UPDATE
terminal); those are extra R12 target-table rows, not additional `blocks` LWTs.
...
0b. **R25 — every known promote path must first establish `pub:`. ✅ SETTLED
   ...
   this is the item to re-read. The "checked" half is R3 (item 8, GREEN).
...
8. **R3 — the `pub:` post-check. ✅ IMPLEMENTED.** After a complete `pub:` stage, require
   an active canonical row, no deleting/handoff/`gc_orphan_handoff=false`/repairing_stub
   claim, and a usable non-orphaned key (`ValidatePublishAttemptAuthority`, `LOCAL_QUORUM`,
   orphan last; orphan-read errors are Unavailable). A failed post-check rolls back the
   `pub:` rows staged by that attempt; rollback failure is recovery/error, never
   publication success. Production stage+promote callers abort on stage error
   (`err != nil` / `nil != err` and a direct return in that if-body),
   including inside `retryLibraryHeadMutation` callbacks.
   Empty batches are vacuously Active. v2 durable repair does not re-run the check.
   R18, R27, R29 integration, R30, R31, Physical ABA and X1 stay OPEN. `GC_ENABLED=false`.
...
10. **R19 / R28 — partial and undiscoverable orphans. ⚠️ PARTLY SETTLED.**
   ...
   token are non-applicable. It also floors its application-clock schedule, skips a
    future `first_seen_at`, and avoids dynamic prepared-statement text. **Two things
    are still open, and none is cosmetic.** First, the rule binds *every* non-creating
    mutation, and only the attempt update has been done: the phase/clear statements
    still key on `(org, block)` rather than on the expected `P` — they cannot key on `P`
    until the physical identity is persisted and threaded through the full lifecycle,
    which is the A+ package. Second, the actual coordinator-clock expiry is not proven
    by the application schedule, and the TTL itself remains, so an orphan can still
    expire and take its recovery record with it; removing it needs the cold-start
    horizon and cursor semantics redefined at the same time, which is R27's unsolved
    partitioning problem, not a one-line schema change. P4b-1's write-once publication
    removes the old reset path but does not by itself bind those later mutations to an
    immutable physical `P`.
11. **R12 — one serial domain prerequisite**, across ten `blocks` statements
    (`r12ExpectedSerialOperations`) and the `gc_s3_orphans` / `gc_block_delete_lifecycles`
    partitions; P0 pins `SERIAL` for the serial phase. `EACH_QUORUM`
    regular visibility, settlement and exact lifecycle identity remain later work
    except where a later slice (P4a/P4b) has already pinned them.
...
| **R19 timestamp guard** | A delayed P1 attempt may not mutate a row with a different observed `first_seen_at` — `TestGC_UpdateS3OrphanAttempt_RejectsDifferentLifecycleToken` | **GREEN for differing tokens.** `StartBlockDeleteOrphan` no longer resets existing lifecycle state; binding later phase/clear mutations to the full physical identity remains open |
...
| R3 safety | A block under an active delete fence, committed handoff, `gc_orphan_handoff=false`, repairing_stub, orphan, missing row, invalid locator or fence-read error must not keep this attempt's `pub:` or continue to HEAD/promote | **GREEN** — unit + `internal/integration/r3_publish_fence_test.go` (`SESAMEFS_REQUIRE_R3_EVIDENCE=1`, including writer-`pub:`-first) + `scripts/r3-publish-fence-mutation-validation.sh` + 3-DC `scripts/x2-multidc-validation.sh --r3`. Stage-error abort is `TestR3ProductionStageErrorsAbortBeforePromote` (`err != nil` + direct return; retry callbacks included). R18 leftover `pub:`, R27, R29 integration, R30, R31 and Physical ABA remain OPEN. |
...
| **Strict A+ non-overlap** (R8's A+ reading: no `P2` while `orphan(P1)` lives) | No `blocks(L) -> P2` may coexist with a live `orphan(L) -> P1` | **OPEN — writer install still does not consult the orphan.** `InstallBlockMetadata` is a plain `INSERT ... IF NOT EXISTS` and never consults `gc_s3_orphans`, so the rowless-mint fence is the READ, not the install CAS. **Through a well-formed lifecycle this is not reachable:** `StartBlockDeleteOrphan` runs strictly before `FinalizeBlockDelete` (`worker.go`), so a writer that reads the canonical row absent is reading after the orphan was durable, and the orphan read that follows observes it — that is exactly what the blocks-first/orphan-last order buys. **It is reachable through a stale destructive lifecycle:** historically `StartBlockDeleteOrphan` never proved the caller still owned the claim. P4a closed claim/release/finalize identity (R14a). **P4b-2 closed the remaining publication path (R14b):** after `CommitBlockDeleteOrphanHandoff`, only stored D can publish or finalize, and a same-P different-D orphan is `DifferentAuthority`. **This row stays OPEN** because `InstallBlockMetadata` is still a plain `INSERT ... IF NOT EXISTS` and never consults `gc_s3_orphans`, so the rowless-mint fence remains the READ, not the install CAS. That writer-install gap is independent of R14b. **Impact if a competing orphan still appears:** `RecoverS3Orphans` checks `BlockExists` first and, finding the canonical row present (now naming `P2`), defers with `continue` before it ever reaches `BlockHasReferencesGlobal` (`worker.go`). Live bytes are never at risk, and unlike the referenced-orphan branch this deferral **does** set `phaseErr`, so the cursor is retained and the row stays in the working set rather than falling out of it as in R18. The result is a stuck fence on `L` that recovery keeps retrying while `P2` lives — bounded in practice by the orphan row's own TTL. |
| **P3 consistency** | GC publishes a writer-visible fence with global commit visibility, and the pre-PUT authority read is linearizable | **GREEN** — `internal/gc/p3_fence_consistency_test.go` pins `EachQuorum` + `Serial` on `ClaimBlockDelete` and `StartBlockDeleteOrphan` as two separate properties; `TestP3RepairAuthorityReadsCanonicalRowBeforeOrphan` pins `BlockAuthorityStrong` at the pre-PUT boundary and `TestP3SerialReadsStayOffTheDedupPath` keeps every other read ordinary. Fence reads pin `LOCAL_QUORUM` rather than inheriting `database.consistency`, which accepts `ONE`; a `ONE` read could otherwise miss a published fence entirely, and `TestP3FenceReadsPinTheirOwnConsistency` fails the build if a `gc_s3_orphans` read stops declaring its level. **Cross-DC evidence, measured:** `scripts/x2-multidc-validation.sh --p3` runs two legs on the real `{dc-na:1, dc-eu:1, dc-asia:1}` fixture. With dc-na stopped, neither `ClaimBlockDelete` nor `StartBlockDeleteOrphan` may publish — Cassandra answers *Cannot achieve consistency level EACH_QUORUM in DC dc-na* — so GC cannot condemn an incarnation whose fence a dc-na writer could not observe. With every DC up, a fence published from dc-eu blocks both the rowless-mint gate and the pre-PUT authority read from dc-na. Both mutations turn the fail-closed leg red: at `LOCAL_QUORUM` and at plain `QUORUM` the publication succeeds while dc-na is blind. **The `QUORUM` mutation is why the fixture has three datacenters** — at two, `QUORUM` is 2 of 2 and fails with a DC down exactly like `EACH_QUORUM`, so the wrong fix would hide. **Still not evidenced:** the pinned READ level. With RF 1 per DC, `LOCAL_QUORUM` and `ONE` both resolve to the single local replica, so the pin defends an RF > 1 deployment this fixture does not have; it stays covered by `TestP3FenceReadConsistencyIsLocalQuorum` and the AST guard. |
...
every known first-publish promote path establishes `pub:` and R3's post-check applies
to those paths, subject to R29's ownership and outcome-preservation contract. v2 durable
repair remains promote-only. The publication TOCTOU between `stage pub:` and `finalize`
is closed for **this attempt continuing** — GC that already authorized on refs==0 cannot
turn that `pub:` into a successful publication. It does **not** close leftover `pub:`
cleanup (R18), retention (R30), or durable repair of a visible HEAD (R31). Read the
status as **R25 fixed, R3 GREEN, R18/R27/R29/R30/R31 OPEN, X1 open**, and do not let a
handshake check be quoted as "publication solved" or as permission to set `GC_ENABLED=true`.
