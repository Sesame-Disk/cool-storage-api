...
8. **R3 — the `pub:` post-check. ✅ IMPLEMENTED.** After a complete `pub:` stage, require
   an active canonical row, no deleting/handoff/repairing_stub claim, and a usable
   non-orphaned key (`ValidatePublishAttemptAuthority`, `LOCAL_QUORUM`, orphan last).
   A failed post-check rolls back the `pub:` rows staged by that attempt; rollback
   failure is recovery/error, never publication success. Empty batches are vacuously
   Active. v2 durable repair does not re-run the check. R18, R27, R29 integration,
   R30, R31, Physical ABA and X1 stay OPEN. `GC_ENABLED=false`.
...
| R3 safety | A block under an active delete fence, committed handoff, repairing_stub, orphan, missing row or invalid locator must not keep this attempt's `pub:` or continue to HEAD/promote | **GREEN** — unit + `internal/integration/r3_publish_fence_test.go` (`SESAMEFS_REQUIRE_R3_EVIDENCE=1`) + `scripts/r3-publish-fence-mutation-validation.sh` + 3-DC `scripts/x2-multidc-validation.sh --r3`. R18 leftover `pub:`, R27, R29 integration, R30, R31 and Physical ABA remain OPEN. |
