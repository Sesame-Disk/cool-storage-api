...
stay as written; they are not the D0 status ledger. X1 remains OPEN.
`GC_ENABLED=false`.
...
- **P4b — claim → orphan publication binding (R14b, R15, R20 orphan path).** ✅ IMPLEMENTED
  ...
  (R3 stays OPEN). `StartBlockDeleteOrphan` INSERTs `gc_block_delete_lifecycles`
