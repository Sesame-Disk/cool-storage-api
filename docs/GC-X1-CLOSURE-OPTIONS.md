...
an ambiguous attempt retains its bytes. Legacy deterministic locators remain
supported. The real Cassandra/MinIO race and AST guards pin these properties.
...
| R9 | Writers in two DCs both leave the wait and both mint a key | **CLOSED by P2.** Exactly one complete tuple becomes canonical through the global-SERIAL install. A direct or serially settled known loser cleans its exact attempted tuple; an ambiguous result retains bytes. The real Cassandra/MinIO race proves winner agreement and preservation. A crash before any durable intent remains X3, not an X1 bucket-inventory requirement. |
