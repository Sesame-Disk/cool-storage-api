...
3. **R21 — orphan authorization provenance. ✅ CLOSED.** `RecordS3Orphan` was removed
   ...
   The conservative A+ option (a) does not depend on historical authorization from the
   orphan, but B and R18(c) can now rely on the provenance invariant when their other
