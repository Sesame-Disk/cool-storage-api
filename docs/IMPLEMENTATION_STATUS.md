...
| **Garbage Collection** | 🟡 IMPLEMENTED / PROD DISABLED | Blocked by X1 | ❌ No | 2026-08-14 | Worker/scanner/lease exist, but keep `GC_ENABLED=false` on every replica/DC until the physical-delete ABA closes; cross-DC reference visibility closed 2026-08-14. Afterward, only designated replicas in one DC may participate under the lease. |
