...
6. Desktop sync active-active conflict recovery - OPEN for desktop GA. `PUT /seafhttp/repo/:repo_id/commit/HEAD` and `POST /seafhttp/repo/:repo_id/update-branch` already use parent-chain validation plus CAS, but both still return `200 OK` on stale conflicts without server-side retry. Functional sync coverage exists, but a real desktop-client conflict/recovery proof under multi-node contention is still missing. This is not a web-only launch blocker.
