...
6. Desktop sync active-active conflict recovery - PARTIALLY VERIFIED for desktop GA. `PUT /seafhttp/repo/:repo_id/commit/HEAD` and `POST /seafhttp/repo/:repo_id/update-branch` now use parent-chain validation, CAS, bounded retry, ancestry-gated server-side auto-merge for safe stale siblings, and `503 + Retry-After` fail-closed responses for unsafe conflicts. `scripts/test-sync-active-active.sh` now proves a real non-overlapping concurrent-write race with observed `parent mismatch` plus `auto-merge`, but the unsafe-conflict `503` path is still not proven end-to-end with a real desktop client.
...
|-----------|--------|-----------|-----------------|---------------|-------|
| **Sync Protocol (Desktop Client)** | ✅ COMPLETE | Mostly stable | ✅ Yes | 2026-05-20 | Functional and comparison tests pass; the real-client active-active harness now proves the non-overlapping auto-merge race, but unsafe-conflict `503` handling is still only handler-covered |
