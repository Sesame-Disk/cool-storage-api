...
| Core Backend API | ~98% | GC implementation exists but destructive GC is production-disabled pending X1; OIDC ✅, Library Settings ✅, Monitoring ✅, Quotas ✅, Plans/Permissions Phase 1+2 ✅, org storage policy backend base ✅ |
...
| Production Infrastructure | ~95% | Destructive GC blocked by X1; Monitoring ✅, Health checks ✅, Structured logging ✅, Frontend/Backend separation ✅, Nginx production hardening ✅ |

**Production Blockers (verified against code 2026-05-20)**:
1. ~~OIDC Authentication~~ - ✅ COMPLETE (Phase 1 - Basic Login)
2. **Garbage Collection activation** - 🔴 BLOCKED: keep `GC_ENABLED=false` on every replica/DC until X1 physical-delete ABA closes. X2 cross-DC reference visibility closed 2026-08-14 under its stable-topology operational contract.
...
7. **GC Multi-Instance Coordination** - ✅ Lease implemented, but activation remains 🔴 BLOCKED — the lease coordinates enabled replicas but cannot close X1. Keep `GC_ENABLED=false` on every replica in every DC. Only after X1 closes may designated replicas in one DC participate under the lease.
...
| **File Upload (REST API)** | ✅ COMPLETE | Mostly stable | ❌ No | 2026-04-06 | Replace/autorename fix: `replace=0` triggers auto-rename (`file (1).ext`), default is overwrite for desktop client compat. Region-aware libraries now propagate `storage_class` as the preference for new materializations through `upload-link` and `seafhttp/upload-api`. |
| **File Download (REST API)** | ✅ COMPLETE | Mostly stable | ❌ No | 2026-04-06 | Optimized: shared `streaming` package — prefetch pipeline, 4MB buffers, batch block resolve, ZIP Store. Existing block reads resolve each block's persisted canonical storage class instead of the current library preference or host default. |
...
| **Garbage Collection** | 🟡 IMPLEMENTED / PROD DISABLED | Blocked by X1 | ❌ No | 2026-09-02 | Worker/scanner/lease exist; keep `GC_ENABLED=false` until X1 closes. Accepted architecture is documented (D0 physical-life handoff) but not implemented. X2 closed 2026-08-14. After X1, only designated replicas in one DC may participate under the lease. |
...
| **Region-Aware Library Storage Selection** | ✅ COMPLETE | Mostly stable | ⚠️ Partial | 2026-04-09 | Safe slice complete: bootstrap exposes region options, personal/group/admin create flows persist `storage_class`, sys-admin and org-admin info pages can manage org `storage_policy`, and new materializations prefer that value. Existing reads, reuse and repairs resolve each block's persisted canonical `blocks.storage_class`; active upload sessions do not snapshot the preference and can mix actual classes. Focused integrations cover user, group, org-admin, and superadmin create flows under `strict` and `flexible` policy modes. Remaining work is explicit migration of existing data, cold-tier primary placement design, and broader multi-region operations. |
| **Monitoring/Health Checks** | 🔒 FROZEN | **STABLE** | ❌ No | 2026-02-04 | Structured logging, `/health`, `/ready`, `/metrics`. 5 unit + 21 integration tests. |
| **Multi-Region Replication** | ❌ TODO | N/A | ❌ No | - | Future feature beyond the current safe slice. Current work prefers a class for new block materializations and keeps historical reads/reuse/repair bound to each block's persisted canonical class; it does not replicate or migrate existing physical data. |
...
- **Status**: Worker, scanner, admin API, and LWT lease are implemented. Keep `GC_ENABLED=false` on every replica in every DC until X1 closes; only then may designated replicas in one DC participate under the lease.
...
- Backend: ~98% for web/API surfaces, but destructive GC is production-disabled pending X1. The distributed lease exists; it coordinates designated participants only after that blocker closes.
...
- Infrastructure: ~95% (monitoring ✅, health checks ✅, GC implementation present but production activation blocked by X1)
