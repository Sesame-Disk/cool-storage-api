...
rows are handled immediately, process memory no longer grows with the total row count, cancellation
can interrupt the query, and a late-page error is surfaced by the phase.
