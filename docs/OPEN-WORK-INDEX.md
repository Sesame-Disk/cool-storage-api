...
| `ISSUE-UPLOAD-PUT-BEFORE-INTENT-01` | MEDIUM | S3 PUT precedes durable intent; minted keys broaden the possible undiscoverable-object leak beyond process crash to ambiguous PUT, unresolved install uncertainty, and failed exact cleanup. Pre-INSTALL retries now retain one target and exact cleanup has a final-outcome metric | Registry X3; bounded request-local cleanup is not a durable outbox, sweeper, or eventual guarantee, so P2 still does not close this issue |
