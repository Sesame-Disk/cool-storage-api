...
| `ISSUE-UPLOAD-PUT-BEFORE-INTENT-01` | MEDIUM | S3 PUT precedes durable intent; minted keys broaden the possible undiscoverable-object leak beyond process crash to ambiguous PUT, pre-INSTALL retry/remint, unresolved install uncertainty, and failed exact-loser cleanup | Registry X3; P2 has no durable attempt table, reconciliation worker, or complete durable observability signal and does not close this issue |
