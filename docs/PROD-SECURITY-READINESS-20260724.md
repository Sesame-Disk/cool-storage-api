...
> **Update 2026-07-25 — NF-1 is fixed.** The share-link password bypass (both the inline-content half and the OnlyOffice download-token half) is closed on `main`; see `ISSUE-SHARELINK-PASSWORD-BYPASS-01` for the shipped fix and its coverage. **The verdict below is unchanged: still no-go as-is**, because B4 (`ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01`) is also single-node reachable and remains open. Per this document's own rule, the verdict text is left as written on its date rather than retro-edited — this note is the delta.
