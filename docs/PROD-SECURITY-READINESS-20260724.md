...
| B4 (subcontract D6) | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **D6 closed, and with it the B4 umbrella.** Per-admission cost was measured rather than estimated: 4.0 MiB for a plaintext stream, 36.0 MiB encrypted — `PrefetchBlock` reads and decrypts whole blocks, so an admitted transfer holds the current and next decrypted block plus their sources — and ~4x/~6x the source for the iWork preview, which buffers the whole document. Against the shipped 1 GiB general preview limit one iWork request could touch six, so §6's sanctioned lever was taken: a `FileView` iWork **source** cap of 32 MiB, which leaves the frozen profile enum, §12 schema and metric label set untouched and avoids throttling ordinary raw streams that cost one block. Capacities divide a stated 2 GiB per-node budget (worst case 6x192 MiB + 18x36 MiB = 1.78 GiB) and ship enabled in all seven configs and both env examples, with every shipped file now loaded through the real validator. Evidence: real `seaf-cli` refused 31 times and still reached `synchronized` with every file pulled and every slot drained; a stalled client through the real frontend nginx held its admission and was released at 1m0s, matching `idle_write_timeout` rather than nginx's 3600s `proxy_read_timeout`; `raw` and `file` live simultaneously under one aggregate with `active_current == sum(active_by_profile)` and `entries_current == active + waiters` holding on a busy node; both directions of public-link/owner isolation, each gated on the other side genuinely refusing first; 200-transfer identity churn draining to baseline; byte-for-byte identical delivery direct and proxied. **Explicitly not closed:** `ISSUE-OBJECT-STORAGE-ANONYMOUS-DOWNLOAD-01`, and `ISSUE-DOWNLOAD-BYTE-RATE-SHAPING-01`, now quantified at 4.9x aggregate scaling over a six-way budget. Throughput figures are loopback numbers and need re-measuring on production hardware. | 2026-08-03 |
