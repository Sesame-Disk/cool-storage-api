...
| B4 (subcontract D3) | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **D3 closed.** Added a reusable idle-write writer that probes connection reachability before headers, refreshes socket deadlines before writes/flushes, cancels after an idle-progress timeout, clears deadlines on completion, and fails closed when `ResponseController` cannot reach the connection. Corrected Go gzip exclusions for the actual `/repo` raw/history paths, `/d/...`, and both public inline bootstrap routes; the supported frontend nginx configuration disables proxy buffering for those bootstrap routes. Focused writer tests include deadline refresh, cancellation, error callbacks, `WriterTo` behavior, unreachable writers and a real TCP slow-client timeout. Producer acquisition/release remains deferred to D4 and production capacities to D6. | 2026-08-02 |
