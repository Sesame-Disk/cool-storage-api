...
| B4 (subcontract B) / X10 | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **Aggregate bound landed; still not closed.** `PutBlock` now takes a per-`(org, user)` and a per-node in-flight admission before `io.ReadAll` and holds it until the handler returns, answering **503 + `Retry-After`** — never 429 — after a bounded wait. The node default `28` is a 2 GiB budget divided by a measured ~73 MiB per concurrent admission; the isolated benchmark's ~40 MiB would have set the cap nearly twice too high, so the end-to-end measurement governs. Six of seven closure criteria met; the seventh — real seaf-cli recovering under saturation — was observed once (22 × 503 absorbed, sync completed, no stranded slots) but has no reliable harness, so B stays open. **B4 remains a blocker: C and D are untouched.** | 2026-07-30 |
| B4 (subcontract B) / X10 | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **Client-recovery criterion closed; hardening audit remains.** A disposable-state Compose harness deterministically occupies both node-3 admissions, asserts a sentinel 503 with positive `Retry-After`, proves real `seaf-cli` is rejected, releases the fault, and requires a subsequent admission, stable sync, byte-for-byte remote payloads, and zero in-flight slots. It passed twice consecutively. Review also found no admitted-request lifetime bound, no waiter cap, no validation of the body-cap × node-cap memory budget, and overstated node/memory evidence; B therefore is not yet certified fully DoS-hardened. **B4 remains a blocker: C and D are untouched.** | 2026-07-30 |
