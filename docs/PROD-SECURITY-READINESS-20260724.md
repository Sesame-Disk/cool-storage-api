...
| Object-storage posture | `ISSUE-OBJECT-STORAGE-ANONYMOUS-DOWNLOAD-01` | **Closed. The 2026-08-01 row above was wrong to call this a production posture item.** The `mc anonymous set download` commands existed only in the four development/test Compose files. Production deploys from `docker-compose.prod.yml`, which contains no MinIO service and no `mc` init container, against provider-native S3 configured in `configs/config.prod.yaml`; AWS and Cloudflare R2 buckets are private by default. No production bucket was ever anonymously readable. The commands are now removed from the dev files — verified that nothing depended on them, since every MinIO consumer in the repository authenticates — and `DEPLOY.md` step 0.1 states the private-bucket requirement explicitly. This does not gate the readiness verdict. | 2026-08-07 |
...
| Sync authentication | `ISSUE-SYNC-LINK-TOKEN-AUTH-01` | **Fixed.** The severity was understated when first recorded. Reproduced live on 2026-08-07 as an *effective* cross-library write: an anonymous share-link visitor replayed its own legitimately-issued download bearer to `PUT /seafhttp/repo/<victim>/block/<sha256>` and the owner's authenticated `check-blocks` confirmed the object landed in a library that was never shared. Two aggravating factors the original row missed: `/download-info` sits inside the same route group and exchanged the path-scoped public bearer for a full repository-root sync token, and nothing bound the token's `RepoID` to the route — the middleware set `repo_id` in the context but no handler ever read it. `isRepositorySyncToken` now validates the whole scope as an allowlist before the bearer becomes an identity: `Source == ""`, `Path == "/"`, and `RepoID` matching `c.Param("repo_id")`. Positive rather than a `Source != "link"` denylist, because a root-directory share link mints a bearer with `Path == "/"` and the right `RepoID` that only the source clause refuses. Each clause mutation-verified: removing it fails exactly one test and no other. Ordinary desktop sync unaffected. | 2026-08-07 |
