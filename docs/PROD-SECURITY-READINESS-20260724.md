...
| B4 (subcontract B) / X10 | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **B closed.** Per-user/node admissions and waiter queues are bounded before body reads; a 5-minute admitted deadline closes stalled bodies, cancels object-storage I/O, and stops Cassandra phases at callback boundaries (each query has the required finite DB timeout); configuration enforces a 64 MiB measured design cost against an explicit 2 GiB budget. Deterministic integration proves both gates. Three clean-process trials reached all 28 slots and measured worst raw correlated RSS/cgroup growth of 49.9 MiB/admission before the 1.25 safety factor. Real `seaf-cli` recovered from the shipped `10s → 503 → Retry-After: 10` cycle, published byte-identical payloads and drained every slot. **B4 remains a blocker only because C and D are untouched.** | 2026-07-30 |
