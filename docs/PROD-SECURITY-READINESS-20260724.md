...
| B4 (subcontract A) / SH-1 | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **Request-rate half closed (A1); A still open.** Anonymous upload-link writes are now bounded per (IP, token) and per token on each node, keyed on token origin so authenticated web uploads on the same route are untouched. Fleet capacity scales with node count. The browser uploaders honour `Retry-After`; without that, resumable.js turned a 429 into a permanently failed file. **A2 — in-flight concurrency — remains unbounded**, and accurate attribution depends on `server.trusted_proxies`. | 2026-07-28 |
