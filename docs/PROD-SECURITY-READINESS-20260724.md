...
| B4 (subcontract A) / SH-1 | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **A1 and A2 closed under the precise post-token subcontract.** After a valid token resolves as link-origin, admission to permission/body/storage work is bounded per (IP, stable link identity) and per link, and by non-blocking process-local in-flight caps with defaults of `16` per stable source and `128` per node. `Source` is unknown before token resolution, so token lookup occurs first; arbitrary invalid-token requests are outside these guards, and A1 does not claim Cassandra lookup protection. Reminting a seafhttp token preserves A1/A2 admission state on that node; authenticated web uploads on the same route remain untouched. These are node-capacity guards, not cluster-global quotas, so aggregate fleet admission scales with node count. The browser uploaders honour `Retry-After`, and accurate client attribution depends on `server.trusted_proxies`. | 2026-07-29 |
