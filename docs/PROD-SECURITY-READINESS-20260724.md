...
| B4 (subcontract A) / SH-1 | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **Request-rate half closed (A1); A still open.** Anonymous upload-link writes are now bounded per (IP, token) and per token, keyed on token origin so authenticated web uploads on the same route are untouched. The browser uploaders were fixed in the same change to honour `Retry-After` — without that, resumable.js turned a 429 into a permanently failed file, so the limiter would have caused an outage rather than prevented one. **A2 — in-flight concurrency — remains unbounded**, and attribution depends on `server.trusted_proxies` being set for the deployment's proxy chain. | 2026-07-28 |
| B4 (subcontract B) / X10 | `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` | **Reduced, not closed.** The `PutBlock` per-request body cap was right-sized 257 MiB → configurable 16 MiB default (ceiling 64 MiB); the old figure came from the web uploader's adaptive-chunk ceiling, which never governed this route. The per-request buffered-body bound drops approximately 16×. **The aggregate bound — the actual defect — is untouched:** N concurrent uploads still cost N × the cap. B4 remains a blocker; A2, C and D remain open. | 2026-07-28 |
