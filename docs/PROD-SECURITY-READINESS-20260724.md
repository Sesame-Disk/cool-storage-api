...
> **Update 2026-07-25 — NF-1 is fixed.** The share-link password bypass (both the inline-content half and the OnlyOffice download-token half) is closed on branch `fix/sharelink-password-bypass` (status of record: ✅ Fixed in `KNOWN_ISSUES.md` / `ISSUE-SHARELINK-PASSWORD-BYPASS-01`). Snapshot rows below that still narrate the open bug (verdict callout, historical NF-1 write-up) are left as dated prose; **live status columns** for SH-6 / checklist item 0 / the blockers table mark it FIXED. **The overall verdict remains no-go as-is** because B4 (`ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01`) is still single-node reachable.
...
|---|---------|------|----------|--------------------------|----------|-------------------|
| ~~NF-1~~ | ~~**Share-link password bypass**~~ — ~~password-protected links serve `fileContent` and an OnlyOffice download token to anonymous callers.~~ | Sharing / Security | HIGH | ✅ **FIXED (2026-07-25)** — gate before content read + OnlyOffice mint; see `ISSUE-SHARELINK-PASSWORD-BYPASS-01` | `ISSUE-SHARELINK-PASSWORD-BYPASS-01` | `sharelink_view.go` `buildShareFileBootstrapResponse`, `buildOnlyOfficeShareBootstrap` |
...
| SH-6 | **NEW (live-confirmed):** password-protected share links leak `fileContent` **and an OnlyOffice download token** without a password on the public bootstrap endpoints — password-control bypass / content disclosure | HIGH | ✅ **FIXED (2026-07-25)** | `ISSUE-SHARELINK-PASSWORD-BYPASS-01` | Gate now runs before the content read and the OnlyOffice mint; helper also fails closed. See **NF-1**. |
...
0. ~~**Share-link password bypass**~~ (`ISSUE-SHARELINK-PASSWORD-BYPASS-01`) — ✅ **DONE (2026-07-25).** Gate inside `buildShareFileBootstrapResponse` before OnlyOffice + text branches; emission drop in the bundle builder; OnlyOffice helper fails closed; regressions cover both branches and both endpoints.
1. **Abuse control** (`ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01` = B4 umbrella; X10 is subcontract B): close all four subcontracts (anonymous seafhttp upload; authenticated block PUT concurrency; check-blocks rate/work amplification; download/GET abuse control) — **blocks go-live** (remaining single-node blocker after NF-1). Closing only block-PUT concurrency does **not** close the umbrella.
...
|------|----------|------|--------|---------------|
| Security posture | B4, B5 | SEC-3 | SEC-1(B2, was High); SEC-4 accepted-for-v1 | OIDC, tokens, secrets, CORS, Accounts admin API-key channel; ~~NF-1~~ fixed |
| Upload | B1 | UP-2 | UP-3/4/5 | UP-6 (max_upload_mb, config-dependent) |
| Download | — | DL-1, DL-2 | DL-4/5/6/7 | DL-3 (fail-open closed) |
| Sharing | — | SH-1 | SH-2/3, SH-5 | scopes, CT-compare, crypto tokens; ~~SH-6 / NF-1~~ fixed |
...
**Net change to the readiness picture:** **no-go as-is.** As of the NF-1 fix (2026-07-25), the remaining single-node blocker is **B4**; **B1/B5** remain multi-instance-only. B2 is a real **Medium** BOLA to fix but not a hard go/no-go. (Historical prose above still narrates NF-1 as discovered; live status is Fixed.)
