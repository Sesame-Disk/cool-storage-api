...
| `CreateFileFromBlocks`, exact session `up:` | Session-owned `up:`, renewed via the same `ensureCommitBlockOwnLiveness` pin/renew step BorrowedFS uses, before session claim | Bounded exact-placement re-validation (`db.ValidateBorrowedFSPublicationAuthority`, LOCAL_QUORUM), now covering SessionUpload as well as BorrowedFS | `pub:` is staged later | `up: near-expiry -> GC claim -> zero-proof -> commit D -> late renewal (does not revoke D) -> pre-HEAD check rejects`; the "TTL alone is not a proof" gap from §14 is closed by renewal ordering, matching W1's BorrowedFS argument | `TestSessionUploadOwnLiveness` (six named legs) and W1's shared `db.ValidateBorrowedFSPublicationAuthority` proof | `CONDITIONAL`; TTL-continuity gap closed for this funnel, R31 remains |
