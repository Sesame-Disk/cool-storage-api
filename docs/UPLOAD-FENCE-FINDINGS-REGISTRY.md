...
| F10 | Medium | **Provisional reference and its expiry are written separately.** A failure between them leaves a reference with no discovery projection, so the zero-ref transition is never found. **PR-8 writes both in one logged batch** (`AddProvisionalBlockReferenceWithExpiry`), which removes the split state instead of compensating for it: the interim PR-3 rollback (release + enqueue on an expiry-write failure) is deleted, and the failure is now plainly retryable. The reference's TTL is derived from `expires_at` inside that call, which is also what F9's scanner change depends on. **Review found F10 had a second half on the release side:** `ReleaseUploadReferences` dropped the reference and then deleted whatever tracker it re-read, so a renewal landing in between lost its tracking while keeping a live reference — the same undiscoverable state, reached from the other direction. Release now goes through `ReleaseProvisionalBlockReference`, which observes the deadline first and retires the tracker only if it still holds it. | `fs_helpers.go`; `provisional_block_ref_expiry.go` | PR-8 |
