...
The X1 row below preserves the finding's physical-identity and claim-ownership reasoning;
it does not adopt r3's generation lifecycle, references, quarantine or abort protocol.
Those were abandoned and are retained only on the reference branch.

| # | Severity | Finding | Tracked as |
|---|---|---|---|
| X1 | Blocker | **Physical delete ABA.** A previously authorized key-only S3 delete can run after the visible fence clears and after byte-identical content is rematerialized. Never-reused physical identity is **necessary** because it prevents a stale delete of `K1` from naming `K2`; it is **not sufficient** because claim ownership and publication fencing remain separate X1 criteria. The current claim id derives from the candidate, so concurrent workers can share it and a stale release can drop another worker's fence. X1 closes only with physical identity, delete authorization bound to that identity, per-attempt claim ownership with stale takeover, and a publication fence that cannot disappear while an authorized delete is in flight. Analysis in `GC-X2-MULTIDC-VALIDATION.md` §3b/§4b. | `ISSUE-GC-UPLOAD-FENCE-REMATERIALIZATION-01` |
| X2 | ✅ **Implemented 2026-08-13, Closed 2026-08-14** | **Cross-DC reference visibility.** `block_references` are ordinary `LOCAL_QUORUM` writes that `SERIAL` does not cover. With RF 1 per DC the write and read quorums need not intersect, so GC in one DC can read zero references for a block that is live in another. The 1h grace period is mitigation, not a bound. **Implemented without r3** by pinning the *authorizing* read to `EACH_QUORUM`: `BlockHasReferencesGlobal` backs `processBlock`'s claim-then-verify, while the pre-claim check, the scanner and `enqueueZeroRefBlocks` stay at session consistency because the zero-check is asymmetric — a local positive is proof, a local zero authorizes nothing. **The write half is pinned at the writers**: both producers (`AddBlockReference` and the logged batch in `AddProvisionalBlockReferenceWithExpiry`) write at `BlockReferenceWriteConsistency` per statement instead of inheriting the session, since the `EACH_QUORUM` read is only as good as the quorum it intersects and `ONE` is an accepted `database.consistency`. This cannot be enforced at the reader — references are written by API nodes, separate processes whose configuration no GC-side gate can observe — so the gate's own consistency check is a second line for writers this binary cannot speak for, and an AST scan (`TestBlockReferenceProducersPinWriteConsistency`) fails on a third producer that forgets. `RemoveBlockReference` is exempt because removal is only ever issued once the referrer has lost authority (a TTL'd publish attempt, or an `fs_object` being deleted); publication/removal races belong to the X1 publication fence. An earlier version of this row justified the exemption with "an under-replicated DELETE leaves the row visible and GC declines to collect" — that is **not** how Cassandra behaves (a DELETE writes a timestamped tombstone, the mutation reaches every replica regardless of level, and last-write-wins means a quorum read touching it resolves to absent), and the wrong reason is what would let someone apply the exemption where it does not hold. `RecoverS3Orphans` performs its own global verify: it *could* inherit authorization transitively from an orphan row that cannot exist unless that verify passed, but that implication only runs forward in time and rested on a greenfield precondition code cannot enforce and that fails silently. A topology gate (`ValidateDestructiveGCTopology`) refuses both destructive paths unless live keyspace replication gives `EACH_QUORUM` per-DC meaning **and the live map equals the declared one** — a map shrunk after references were acknowledged elsewhere passes every structural check while `EACH_QUORUM` stops being obliged to reach them. The gate lives in `GCStore`, so dropping it is a compile error. Needed **no** generations, physical incarnations, **additional writer round trip, or WAN consistency on the shipped upload path**, and no `SERIAL+ALL`. (This row used to say "no writer hot-path change"; that stopped being true when the producer pin landed — reference writes now name their consistency explicitly, and the pin is a ceiling as well as a floor, lowering a deployment configured for `EACH_QUORUM`/`ALL` to `LOCAL_QUORUM`.) — that fence serves the publication TOCTOU, a different property. **Closure evidence (2026-08-14):** all five legs on `docker-compose.cassandra-3dc.yaml` at RF 1 ran green in one pass — `LOCAL_QUORUM(dc-na)=false` and `EACH_QUORUM(dc-na)=true` against the same divergent state (leg 1), fail-closed with a DC down (leg 2), fail-closed with the DC holding the **only** reference down (leg 2b), and both gate halves (legs 3a/3b). **Both mutations were run against that code**: `EACH_QUORUM→LOCAL_QUORUM` turns leg 1 red, and `EACH_QUORUM→QUORUM` turns leg 2b red with "the destructive read returned zero references while dc-eu — the only datacenter holding one — was unreachable". Leg 2b plus mutation B is what makes the three-DC fixture load-bearing rather than decorative: it is the only combination that shows the *wrong* fix returning a false zero on live data. Two DCs reproduce the defect but cannot rule out the wrong fix: a non-local `QUORUM` is 2 of 2 there, so it intersects by accident and looks as good as `EACH_QUORUM`; only at three does it become 2 of 3 and able to miss the replica holding the reference. **X2 closed does not enable GC** — `GC_ENABLED=false` stands; X1 is now the sole runtime activation blocker. | `ISSUE-GC-CROSS-DC-REFERENCE-VISIBILITY-01` |
...
| X4 | High (perf) | **One `SERIAL` LWT/Paxos transaction per block invocation that reaches metadata registration.** The first-writer `INSERT ... IF NOT EXISTS` is a per-block LWT; when the effective setting is `SERIAL` and the replica topology spans DCs, it is a cross-DC serial transaction. New content/full registration is ~128 such transactions per GiB at the 8 MiB block size; the number of network round-trips depends on Cassandra's Paxos variant. **Correction:** an earlier revision said the legacy resumable path "does not pay this at all". That is false — `finalizeUploadStreaming` splits a resumable upload into 8 MiB blocks and calls `RegisterUploadedBlock` → `UpsertBlockMetadata` per block, so resumable pays the same cost when it reaches registration. Browser/sync preflight can classify a fully deduplicated block before that helper, so the figure is not a universal per-file cost. **Note for any X1 design that mints storage keys:** P-4's proposed fix — drop the first-writer LWT once storage class is deterministic — stops being available, because a minted key makes the LWT the only thing that picks one canonical incarnation across DCs (R9 in `GC-X1-CLOSURE-OPTIONS.md`). The pre-store `ProbeBlockReuse` and post-reference `BlockDeleteFenceActive` observations are not redundant and must not be merged: their ordering is the mutual exclusion the protocol depends on. PR-5 adds a third post-materialization confirmation observation to close fast-clear; it is also required and must not be served from either earlier result. Only the LWT is removable here. **Pre-existing since `e3883aa5d` (2026-05-28)** — not introduced by this work; `13e01263` only made it representation-aware. | `ISSUE-UPLOAD-PER-BLOCK-PAXOS-01`; P-4 in `UPLOAD-PERFORMANCE-SECURITY-2026-06.md`; PR-11 (deferred) |
...
The deterministic-placement/LWT-removal language in X4 is a historical proposal,
not the current product recommendation. The current reference remains mutable
library preference plus an org-global canonical block row; any B/C redesign must
be approved with its identity, locator and GC-liveness proof before the LWT can be
removed.

### Dated note — X4 / X1 hot-path characterization, 2026-08-20

The [X1/X4 upload hot-path characterization](./UPLOAD-PAXOS-HOT-PATH-X1-CHARACTERIZATION.md)
confirms that the shipped production default/example uses `SERIAL` for the
metadata LWT, subject to environment overrides and the effective replica
topology. P0/R12 makes the domain explicit rather than introducing its cost to
deployments already using `SERIAL`.
...
that permit. Chunked workers retain their eight-slot capacity while waiting for
the permit; the two-minute final-file context starts after `eg.Wait()`, and
...
### Dated note — X4 scope correction, 2026-08-11

X4 applies to each block invocation that reaches canonical metadata materialization,
not literally every logical block of every upload. Browser and sync preflight can
classify a fully deduplicated block before `RegisterUploadedBlock`, in which case that
block pays no metadata LWT. First upload of new content, a first-writer race,
rematerialization, and reusable-row results that do reach the helper still pay the
first-writer LWT. The ~128 LWT/Paxos transactions per GiB figure is therefore a
new-content/full-registration sensitivity at 8 MiB blocks, not a universal cost for every 1 GiB
upload. The probe fast path helps existing complete rows only; it cannot remove the
first-content linearization decision.

### Dated note — X1 design direction, 2026-08-14

The generational-fence protocol (r3) drafted on `docs/gc-x1-x2-generation-fence-final`
is **abandoned**, and PR #166 closed unmerged. It is kept as investigative reference on
that branch only; no part of it is a decision of record. X1 is now being evaluated against a
much smaller design — never-reused physical storage keys with the logical SHA-256 kept
as the deduplication identity — documented with its full option comparison, race matrix
and open questions in [GC-X1-CLOSURE-OPTIONS.md](./GC-X1-CLOSURE-OPTIONS.md). Nothing
there is accepted yet and no implementation has started; `GC_ENABLED=false` stands.
