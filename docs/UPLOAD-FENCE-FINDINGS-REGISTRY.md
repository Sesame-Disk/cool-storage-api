...
byte, so a literal reading files them at Medium regardless of how much damage
they do. X10 is rated **High** to match the production-blocker status of its
umbrella `ISSUE-RATE-LIMIT-UPLOAD-DOWNLOAD-01`, not because it fits the "High"
wording above. Read availability severities from `KNOWN_ISSUES.md`, which rates
across both axes; do not re-derive them from this scale.
