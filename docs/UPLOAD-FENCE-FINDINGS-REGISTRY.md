...
| F10 | Medium | **Provisional reference and its expiry are written separately.** A failure between them leaves a reference with no discovery projection, so the zero-ref transition is never found. **PR-8 writes both in one logged batch** (`AddProvisionalBlockReferenceWithExpiry`), which removes the split state instead of compensating for it: the interim PR-3 rollback (release + enqueue on an expiry-write failure) is deleted, and the failure is now plainly retryable. The reference's TTL is derived from `expires_at` inside that call, which is also what F9's scanner change depends on. **Review found F10 had a second half on the release side:** `ReleaseUploadReferences` dropped the reference and then deleted whatever tracker it re-read, so a renewal landing in between lost its tracking while keeping a live reference — the same undiscoverable state, reached from the other direction. Release now goes through `ReleaseProvisionalBlockReference`, which observes **both** identities up front and conditions **both** deletes — the reference on its `created_at`, the tracker on its `expires_at`. Conditioning only the tracker (the first attempt) still let a renewal lose its *reference*, after which the caller saw zero references and promoted a live upload's block to a GC candidate. Partial-failure handling too: a projection-cleanup failure no longer costs the zero-reference transition, and Phase 0's canonical-missing branch promotes an unreferenced block before sweeping its projection. | `fs_helpers.go`; `provisional_block_ref_expiry.go`; `block_references.go`; `gc/scanner.go` | PR-8 |
