...
| F10 | Medium | **Provisional reference and its expiry are written separately.** A failure between them leaves a reference with no discovery projection, so the zero-ref transition is never found. **PR-8 writes both in one logged batch** (`AddProvisionalBlockReferenceWithExpiry`), which removes the split state instead of compensating for it: the interim PR-3 rollback (release + enqueue on an expiry-write failure) is deleted, and the failure is now plainly retryable. The reference's TTL is derived from `expires_at` inside that call, which is also what F9's scanner change depends on. **Review found F10 had a second half on the release side:** `ReleaseUploadReferences` dropped the reference and then deleted whatever tracker it re-read, so a renewal landing in between lost its tracking while keeping a live reference — the same undiscoverable state, reached from the other direction. Release now goes through `ReleaseProvisionalBlockReference`, which reads ONE shared `generation_id` (migration 013, stamped on both rows in the same batch) and deletes only the reference matching it — then leaves the tracker to Phase 0. Two earlier attempts failed to narrower windows: conditioning only the tracker let a renewal lose its *reference*; conditioning both against each row's own timestamps allowed a **mixed observation** across the two reads (and millisecond collisions). Leaving tracking to Phase 0 is also what makes the zero-reference transition durable — a crash or failed enqueue after the release no longer erases the block's last discovery path. Phase 0's canonical-missing branch additionally promotes an unreferenced block before sweeping its projection. | `fs_helpers.go`; `provisional_block_ref_expiry.go`; `block_references.go`; `gc/scanner.go`; migration `013` | PR-8 |
