...
//     is — no per-event array growth / shift() churn.
...
export function createUploadThroughputMeter({ windowMs = 3000, bucketMs = 250, now = Date.now } = {}) {
    ...
    // bytes recorded BEFORE this bucket, so bytes-in-window = cumulative - buckets[0].startCumulative.
    let buckets = [];
    ...
    return {
        ...
        addBytes(delta, at = now()) {
            ...
            if (!last || last.t !== bucketStart) {
                buckets.push({ t: bucketStart, startCumulative: cumulative });
            }
            cumulative += d;
            prune(at);
        },
        ...
        rate(at = now(), { minSpanMs = 0 } = {}) {
            ...
            prune(at);
            const last = buckets[buckets.length - 1];
            // Real inactivity: no bytes for a full window (measured from the last bucket's end).
            if (at - (last.t + bucketMs) >= windowMs) {
                return 0;
            }
