...
func TestNoLibrariesHeadCommitIDWriterBypassesPublicationAuthority(t *testing.T) {
	...
	const wantWriters = 2 // createInitialCommit, executeLibraryHeadCAS -- one CAS each; legacy-NULL retry was removed (docs/CHANGELOG.md "W2a pre-merge closure round 3": clean-deploy codebase, no legacy dataset)
	if totalFound != wantWriters {
		t.Fatalf("found %d libraries.head_commit_id writer statements in package api, want %d -- update this count (and re-audit each writer) if a head_commit_id writer was added, removed, or restructured", totalFound, wantWriters)
	}
}
