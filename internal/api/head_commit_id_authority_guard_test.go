...
// statements) -- see internal/api/v2/libraries.go's UpdateLibrary handler for
// an example of that shape, which today never touches head_commit_id. If a
...
func TestNoLibrariesHeadCommitIDWriterBypassesPublicationAuthority(t *testing.T) {
	...
	const wantWriters = 4 // createInitialCommit (primary + legacy retry), executeLibraryHeadCAS (primary + legacy retry)
	if checked != wantWriters {
		t.Fatalf("found %d libraries.head_commit_id writer statements in package api, want %d -- update this count (and re-audit each writer) if a head_commit_id writer was added, removed, or restructured", checked, wantWriters)
	}
}
