...
func (h *OrgAdminHandler) AddOrgGroupOwnedLibrary(c *gin.Context) {
	...
	batch.Query(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, targetOrgID, newLibID, callerUserID, repoName, false, blockRepresentationID, resolvedStorageClass, int64(0), int64(0), dbpkg.LibraryPublicationStateActive, now, now)
	batch.Query(`
