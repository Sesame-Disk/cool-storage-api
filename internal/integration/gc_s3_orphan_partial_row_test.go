...
func TestGC_UpdateS3OrphanAttempt_DoesNotResurrectClearedOrphan(t *testing.T) {
	...
	if _, err := store.StartBlockDeleteOrphan(orgID, blockID, "hot", syntheticCanonicalStorageKeyForTest(orgID, blockID), "", firstSeenAt); err != nil {
...
func TestGC_UpdateS3OrphanAttempt_RejectsDifferentLifecycleToken(t *testing.T) {
	...
	if _, err := store.StartBlockDeleteOrphan(orgID, blockID, "hot", syntheticCanonicalStorageKeyForTest(orgID, blockID+"-p1"), "sha1-p1", p1FirstSeenAt); err != nil {
	...
	if _, err := store.StartBlockDeleteOrphan(orgID, blockID, "cold", syntheticCanonicalStorageKeyForTest(orgID, blockID+"-p2"), "sha1-p2", p2FirstSeenAt); err != nil {
