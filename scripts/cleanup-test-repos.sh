...
cleanup_for_token() {
  ...
  echo "$repos_json" | jq -r '.repos[]? | select(.repo_name | test("^(batch-ops-test-|nested-move-copy-test|FileOpsTest-|HistoryTest-|cross-lib-src-|cross-lib-dst-|with-parents-test$|api-token-test-|tag-test-library-|sa-test-lib-|sync-test-|sync-aa-|inttest-|smoke-|sesamefs-public-smoke$|test-)")) | .repo_id' | while read -r repo_id; do
    if [ -n "$repo_id" ]; then
